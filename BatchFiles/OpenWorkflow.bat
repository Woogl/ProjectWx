...
"%WX_WIKI_PWSH%" -NoProfile -ExecutionPolicy Bypass -File "%~dp0..\.agents\scripts\Start-WorkflowServer.ps1"
if errorlevel 1 echo AI connection unavailable. Document browsing is still available.
"%WX_WIKI_PWSH%" -NoProfile -ExecutionPolicy Bypass -File "%~dp0..\.agents\scripts\Export-WorkflowPage.ps1" -Open
if errorlevel 1 (
  pause
  exit /b 1
)
