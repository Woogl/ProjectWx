...
- 절차:
  ...
  4. `Wiki/` 변경만 커밋해 `git push origin HEAD:main`으로 올립니다. 커밋 메시지는 `Wiki 갱신: <바뀐 내용 한국어 요약>`입니다. 바뀐 것이 없으면 커밋하지 않습니다.
     - 다른 갱신이 먼저 올라가 푸시가 거절되면 합치지 않습니다. 레저를 손으로 합치면 어긋날 수 있기 때문입니다. `git fetch origin main`과 `git reset --hard origin/main`으로 되돌린 뒤 1단계부터 다시 합니다. 이미 반영된 원자료는 해시 대조로 건너뜁니다. force push는 하지 않습니다.
...
- AI: 대시보드 왼쪽 메뉴 아래 「처리할 AI」에서 고른 AI(Codex·Claude Code·Gemini CLI 중 연결된 것)가 Windows에서 그대로 갱신합니다. WSL에는 AI를 설치하지 않습니다.
