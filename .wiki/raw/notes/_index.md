...
| [NameplateManager를 WxGame으로 옮기고 마커·락온 질의를 제거](2026-09-24-nameplate-manager-wxgame.md) | WxUI의 UWxNameplateManagerComponent를 WxGame Controller로 옮겨 적과 락온 대상을 직접 읽게 했다. LockOnTargetQuery·PC 연결 코드·UWxNameplateSourceComponent·VisibilityRequirements를 지웠고 높이는 캡슐 윗면 기준으로 바꿨다. State.Engaged는 유지한다. 인게임은 미검증. | wx, ui, game, architecture | 2026-09-24 |
| [모듈화 목적 재정의와 배치 원칙 정립](2026-09-24-module-principles.md) | Wx 플러그인의 목적을 다른 프로젝트 재사용에서 게임 내부 도메인 경계로 다시 정했다. 초안을 기존 결정·코드로 반증해 책임 기반 배치, 성격별 통로, 계약 조건, 기능 단위 조립 허용, WxCore 상태 없는 헬퍼 허용으로 고쳐 확정했다. 베이스 클래스 사례는 보류했다. | wx, architecture, foundation | 2026-09-24 |
...
| [WxCombat 불필요한 장치 정리: 쿨다운 무시 순정화·사망 BT 정지 단일화·락온 대상 사본 제거](2026-09-24-wxcombat-machinery-cleanup.md) | 소환물의 쿨다운 무시를 순정 Immunity·RemoveOther GE(UWxEffect_IgnoreCooldowns)로 바꾸고 NoCooldown·AbilityBase 태그 분기를 지웠다. 사망 BT 정지는 AI 컨트롤러만 하고, 락온 태스크는 UWxLockOnComponent의 대상을 매 틱 읽는다. 쓰지 않는 SendGameplayEvent 노티파이·InfiniteMP GE와 ASC 발동 실패 로그를 지웠다. 빌드 통과, 인게임 미검증. | wx, combat, ai, architecture | 2026-09-24 |
| [플레이어 래그돌 떨림: 마네킹 메시의 PhysicsAsset을 적과 같은 것으로 통일](2026-09-24-ragdoll-physics-asset.md) | 플레이어가 쓰는 Quinn 메시의 /Game/Mannequins/Rigs/PA_Mannequin은 기본 자세에서 겹치는 바디 쌍의 충돌이 켜져 래그돌이 떨렸다. SKM_Manny만 바꿨던 2026-04-22 교체를 SKM_Quinn_Simple·SKM_Quinn·SKM_Manny_Simple에도 적용했다. 충돌 비활성 표 추출·겹침 계산 방법 포함. 인게임 미검증. | wx, game, physics, ragdoll | 2026-09-24 |
