...
volatility: warm
verified: 2026-09-26
summary: "Wiki는 프로젝트 지식의 정본이고, 작업 절차·기록 형식·상태 규칙의 정본은 작업 절차 문서 한 장이다. 이 문서는 규칙을 다시 적지 않고 Workflow 도구(대시보드·로컬 서버·실행기·처리 프롬프트)의 구조와 동작, 결정 이유를 다룬다."
---

# Wiki·Workflow 도구 구조

Wiki는 프로젝트 지식의 정본이고, 작업 절차·기록 형식·상태 규칙의 정본은 [작업 절차](../../../.agents/workflow/process/index.md) 한 장이다. 이 문서는 절차 규칙을 다시 적지 않는다. 대신 Workflow 도구(대시보드·로컬 서버·실행기·처리 프롬프트)의 구조와 동작, 결정 이유를 다룬다. 작업 현황은 각 기록 제목 아래 상태 줄에서 만든다. 작업은 대시보드의 새 작업·작업 진행이나 AI 대화 어느 쪽에서도 시작하고 이어가며, 둘 다 같은 기록 절을 주고받는다.
...
작업 절차·기록 형식·상태·테스트 체크리스트 규칙은 [작업 절차](../../../.agents/workflow/process/index.md) 한 장에만 둔다. 2026-09-26 사용자 결정("워크플로우의 SSoT를 엄격하게 지킵시다")에 따라 AGENTS.md와 이 Wiki는 규칙을 다시 적지 않고 그 문서를 가리킨다([정리 경위](../../raw/notes/2026-09-26-workflow-ssot-copies.md)).

웹 처리 프롬프트(`Workflow-TestFeedback.cjs`의 `taskPrompt`)도 먼저 AGENTS.md와 작업 절차를 따르게 한다. 그 밖에는 규칙이 아닌 다음 내용만 적는다.

- 이번 처리의 단계
- 결과 칸 형식
- 전체 권한 실행의 제한: 관리자 정책·CLI 설정·커밋·푸시·외부 메시지 금지, 기존 사용자 변경 보존
- 웹 전용 약속: 작업 기록과 접수 JSON은 서버가 쓴다

전에는 체크리스트·질문 규칙을 프롬프트에 다시 적었는데, 절차를 바꿀 때마다 프롬프트도 같이 고쳐야 했다. 사람 항목 보존, 미실행 항목 넘김, 사람 항목 최소 하나는 프롬프트 문장이 아니라 서버 코드가 강제한다(아래 체크리스트 절). 터미널에서 이어하기의 고정 요청문도 기록 경로와 AGENTS.md·작업 절차만 가리킨다.
...
서버는 AI 결과를 단계마다 정해진 칸으로만 받는다: 정하기는 질문·계획, 구현과 수정은 변경·질문·체크리스트, 추가 요청은 변경·질문·계획·체크리스트, 완료 뒤 정리는 변경뿐이며 모두 요약·근거를 포함한다. CLI에 넘기는 스키마도 단계별이고 서버는 허용된 칸만 읽으므로, AI가 다른 칸을 채워도 기록에 들어가지 않는다(첫 실제 처리에서 정리 결과의 계획 칸 문장이 승인 없는 계획 절이 된 사례). 구현 중 판단이 필요하면 질문으로 묻고, 답하면 정하기로 돌아가 새 계획과 승인을 거친다. AI가 돌려준 질문은 미답변으로 붙이고 겹치는 ID는 새 번호를 준다. 계획의 `#` 줄과 `구현 승인:` 줄은 목록 항목으로 바꿔 절 구조를 깨거나 AI가 스스로 승인할 수 없다.

## 테스트 체크리스트와 결과 전달

체크리스트 표 형식과 작성 규칙은 [작업 절차](../../../.agents/workflow/process/index.md)의 테스트 체크리스트 절에 있다. 해석기([task-records.js](../../../.agents/scripts/wiki-viewer/task-records.js))는 머리글과 행 형식을 엄격하게 검사하므로, 형식이 어긋난 표는 체크리스트로 읽히지 않는다.
