...
미러링은 원본 스펙의 클래스·레벨만 자기 ASC에 부여하고 `TryActivateAbility`로 실행한다. `ABS_Doppelganger`의 `UWxEffect_IgnoreAbilityActivationTags`가 소유자 발동 조건을 면제하므로 Master에게 성립한 GA 선택 조건을 도플갱어 상태로 다시 요구하지 않는다. 어빌리티·GE 차단과 도플갱어 자신의 본동작·콤보·후딜은 유지한다. 비용·쿨다운 무시는 기존 별도 효과다. 상세 계약은 [[combat-abilities|전투 어빌리티와 이펙트]] ([전투 어빌리티와 이펙트](../concepts/combat-abilities.md))를 따른다.

도플갱어의 콤보 창이 아직 열리지 않았다면 기존 `RetryDuration` 기본값 0.4초 동안 재시도한다. 최신 실패 요청 하나만 보관하므로 긴 지연이나 연속된 여러 요청의 완전 동기화를 보장하지 않는다. 이번 태그 차단 전환에서는 미러링 태스크를 변경하지 않았다.
