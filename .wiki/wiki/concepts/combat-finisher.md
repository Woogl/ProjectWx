...
flowchart LR
  ...
  D --> E["공격자 워프 타겟·짝 몽타주·피해 준비"]
  E --> F["종료: 피해 정리·대상 GP 초기화"]
```
...
4. 공격자는 ServerInitiated/Override로 활성화되고 무적 효과를 소유한다. 서버는 피해자에게 짝 몽타주 어빌리티를 일회 부여한다. 공격자에게 `Finisher` 워프 타겟을 등록하고 피해 컴포넌트를 시작한다.
5. 종료 때 피해 컴포넌트를 정리하고 서버에서 대상 GP 초기화 효과를 적용한다. GP 초기화는 이 C++ 경로상 피해 노티파이 순간이 아니라 어빌리티 종료에 놓여 있으며 취소 종료도 포함한다.
...
진입점: [적 상호작용](../../../Source/WxGame/Character/WxEnemyCharacter.cpp), [서버 검증](../../../Source/WxGame/AbilitySystem/Ability/WxAbility_Interact.cpp), [피니시 어빌리티](../../../Plugins/WxCombat/Source/WxCombat/Private/AbilitySystem/Ability/WxAbility_Finisher.cpp).
