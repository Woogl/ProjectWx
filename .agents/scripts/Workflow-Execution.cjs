...
AGENTS.md와 .agents/wiki/index.md를 읽고 다음 확정 인계에 따라 ${record.phase==='implement'?'코드를 구현하고 필요한 빌드·회귀 검증과 자체 코드 리뷰를 수행':'사람이 코드 리뷰한 구현에 대해 기획의 완료 기준과 실제 테스트 시나리오를 검증'}하세요.
