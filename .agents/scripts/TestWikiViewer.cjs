...
assert.equal(byId('work-summary').children.filter(n => n.tagName === 'A').length, 4);
assert.equal(byId('work-summary').children.length,4,'only stage cards belong in the grid');
assert.match(byId('current-task-title').textContent,/새 작업/);
assert.equal(byId('work-summary').children[0].children[1].textContent,'기획서 입력부터 시작');
assert.equal(byId('work-summary').children[1].children[1].textContent,'기획 확정 후 진행');
...
run('sharedReady=true;flushSharedTasks=async()=>{draftRecovery=Object.create(null)};loadSharedTasks=async()=>{}');
...
(async()=>{
  ...
  assert.ok(input('추가 요청·수정 의견'),'feedback is available even with no questions');
  ...
  assert.equal(control('기획 최종 확정 · 인계 문서 저장').disabled,true,'unresolved decision disables handoff');
  ...
  assert.equal(control('기획 최종 확정 · 인계 문서 저장').disabled,false);
  ...
  const saving=control('기획 최종 확정 · 인계 문서 저장').onclick();
  await new Promise(resolve=>setImmediate(resolve));
  assert.equal(byId('workflow-busy').hidden,false);
  assert.equal(byId('workflow-busy-title').textContent,'인계 기록을 저장하고 있습니다');
  releaseSave();await saving;context.fetch=saveFetch;
  assert.equal(byId('workflow-busy').hidden,true);
  assert.ok(descendants(byId('workflow-controls')).some(n=>n.textContent==='저장 완료 · .agents/in-progress/test-1.md'));
  ...
  await control('기획 최종 확정 · 인계 문서 저장').onclick();
  assert.equal(run("loopReady('implementation')"),false,'same draft with a new upstream decision record still requires design rereview');
  ...
  assert.equal(control('기획 최종 확정 · 인계 문서 저장').disabled,true);
  ...
  input('CHANGE 답변').value='10으로 변경';input('CHANGE 답변').oninput();await control('이 답변으로 결정').onclick();
  response={...result,draft:'회피 비용 10 설계'};
  await control('AI 설계 검토 · 답변 반영').onclick();applyExclusions();
  assert.equal(run("loopReady('implementation')"),false,'planning change cannot finalize in design');
  await control('기획 변경 질문 함께 전달 · 답변 보존').onclick();
  ...
  await control('AI 기획 검토 · 답변 반영').onclick();applyExclusions();
  await control('기획 최종 확정 · 인계 문서 저장').onclick();
  assert.equal(run("loopReady('implementation')"),false,'new planning requires design rereview');
  run("renderWorkflow('.agents/wiki/workflow/implementation.md')");
  await control('AI 설계 검토 · 답변 반영').onclick();applyExclusions();
