...
function executionStatus(){
  ...
  return ({running:execution.phase==='verify'?'AI 검증 중':'AI 구현 중',review:'코드 리뷰 필요',acceptance:'결과 확인 필요',complete:'완료',blocked:'AI 작업 확인 필요',interrupted:'AI 작업 중단'})[execution.status]||'확인 필요';
}
function currentWorkStage(){
  if(!loopConfirmed('planning'))return 'planning';
  if(workflowState.execution?.status==='complete')return 'completion';
...
async function executeAction(action,feedback='',confirmedChecks=[]){
  if(executionRequestBusy)return;
  ...
  try{
    ...
    if(!request){request={taskId,action,feedback,confirmedChecks,expectedRevision:workflowState.execution?.revision||0,operationId:requestId()};localStorage.setItem(key,JSON.stringify(request));}
...
function renderExecution(panel){
  ...
  if(execution.status==='complete'){
    panel.append(el('p','구현·검증 결과와 사람의 수용 판단이 저장되었습니다.'));
    for(const decision of execution.decisions)panel.append(el('p',(decision.action==='approve'?'코드 리뷰 승인':'최종 수용')+' · '+decision.at+(decision.feedback?' · '+decision.feedback:''),'notice'));
    return;
  }
  const judgment=workflowState.executionJudgment?.revision===execution.revision?workflowState.executionJudgment:{revision:execution.revision,feedback:'',checks:[]};
