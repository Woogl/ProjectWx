...
$revision = (@($script, (Join-Path $PSScriptRoot 'wiki-checklist.schema.json'), (Join-Path $PSScriptRoot 'wiki-viewer/workflow-model.js'), (Join-Path $PSScriptRoot 'Wiki-AI-Providers.cjs'), (Join-Path $PSScriptRoot 'wiki-gemini-policy.toml'), (Join-Path $PSScriptRoot 'wiki-gemini-settings.json'), $PSCommandPath, (Join-Path $PSScriptRoot 'Wiki-Import.cjs'), (Join-Path $PSScriptRoot 'Wiki-Import.py'), (Join-Path $PSScriptRoot 'Wiki-Tasks.cjs'), (Join-Path $PSScriptRoot 'Workflow-Execution.cjs'), (Join-Path $PSScriptRoot 'Workflow-TestFeedback.cjs')) | ForEach-Object { (Get-FileHash -LiteralPath $_ -Algorithm SHA256).Hash.ToLowerInvariant() }) -join ':'
