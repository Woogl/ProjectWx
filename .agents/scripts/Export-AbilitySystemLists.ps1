...
function Get-AbilityBody($Abilities, $Montages, $Types) {
    ...
    $lines.Add('## 몽타주')
    $lines.Add('')
    $lines.Add('GA_가 쓰는 몽타주와 그 몽타주가 참조하는 몽타주다. 섹션은 시작 시각 순이다. 노티파이는 클래스별로 묶어 처음 나오는 시각 순으로 적고 `×N`은 개수다. `{}` 안은 그 클래스 인스턴스들에 저장된 값을 필드별로 중복 없이 모은 것이라, 어느 인스턴스·섹션의 값인지와 모든 인스턴스가 그 값을 갖는지는 나타내지 않는다. 피해 행 값은 [이펙트 목록](../references/effect-list.md)에 있다.')
    $lines.Add('')
