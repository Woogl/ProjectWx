...
**경계 (비담당)**
- 대화 창 위젯의 개폐·표시 — `State.Dialogue` 태그 전이를 듣는 [WxUI](WxUI.md) 의 UI 매니저가 판단하고, 대사 바인딩은 [WxGame](WxGame.md) 의 `UWxViewModel_Dialogue` 가 `OnLineChanged` 를 받아 처리한다. 모듈은 UI 타입을 전혀 모른다
- 상호작용 감지·입력·차단 — `IWxInteractable` 계약 자체는 [WxCore](WxCore.md) 가 정의하고, `State.Dialogue` 로 상호작용 어빌리티를 막는 것은 [WxGame](WxGame.md) 쪽이다
- "말을 걸 수 있는가"의 조건 판정(`CanInteract`) — 퀘스트 대기 상태를 봐야 하므로 [WxGame](WxGame.md) 의 `AWxNpc` 가 답한다
- 대사가 무슨 의미인지(퀘스트 수주·보상 등)의 해석 — 종료를 기다린 쪽([WxQuest](WxQuest.md) 등)이 판정한다. 대화는 기록을 남기지 않는다
- 네이티브 태그 선언 — `State.Dialogue` 는 [WxCore](WxCore.md) 의 `WxGameplayTags.h` 소유
- NPC 외형/골격 구성 — [WxGame](WxGame.md) 의 `AWxNpc`
