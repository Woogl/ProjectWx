...
**경계 (비담당)**
- 게임플레이 태그·`IWxUIData`(표시 이름/아이콘) 정의는 [WxCore](WxCore.md). 본 모듈은 `WxGameplayTags::UI_Layer_*`·`UI_Action_*` 를 **사용만** 한다.
- 어빌리티·이펙트·어트리뷰트의 실제 로직은 [WxCombat](WxCombat.md). 여기서는 ASC 를 읽어 표시로 바꾸기만 한다.
- 상호작용 후보 탐색은 [WxWorld](WxWorld.md)의 스캐너가, 그 목록 VM(`UWxViewModel_InteractionList`)은 `WxGame` 이 소유한다. 본 모듈은 항목 하나짜리 `UWxViewModel_Interaction` 만 제공한다.
- 자막·인디케이터를 *언제* 띄울지는 [WxQuest](WxQuest.md) 등 소비 도메인의 StateTree 에셋이 정한다. 본 모듈은 노드와 표시 수단만 제공한다.
