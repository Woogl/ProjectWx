...
**경계 (비담당)**
- 저널을 화면에 그리는 일 — 뷰모델·위젯은 `WxGame`(`UWxViewModel_Quest`)과 [WxUI](WxUI.md)가 담당하며, 이 모듈은 `OnJournalChanged` 통지만 쏜다
- 퀘스트 컴포넌트를 GameState 에 붙이는 일 — `WxGame` 의 `AWxGameState` 생성자가 기본 서브오브젝트로 생성한다
- 보상 지급·전투·대화 등 퀘스트가 걸어 놓는 실제 게임 동작 — [WxCombat](WxCombat.md), [WxInventory](WxInventory.md), [WxDialogue](WxDialogue.md) 등 각 도메인의 StateTree 노드로 조립한다
