...
WxCore는 도메인 간에 공유하는 GameplayTag·인터페이스·충돌 채널을 정의합니다.
...
## 책임
**담당**
- 프로젝트 C++ Native Gameplay Tag의 유일한 선언처. 도메인 플러그인은 자체 태그를 선언하지 않는다(2026-09-21 확인)
- 도메인 간 경계를 넘는 인터페이스 계약(`IWxInteractable`, `IWxUIData`, `IWxSpawnable`) 정의와 기본 구현
- 공격 판정용 커스텀 Object Channel 상수(`ECC_WxAttack`)의 C++ 측 고정
- 에디터 전용 로케이터 표시명 헬퍼

**경계 (비담당)**
- 태그를 실제로 붙이고 떼고 소비하는 주체 — 전투·리액션은 [WxCombat](../combat/index.md), 상호작용 실행은 [WxWorld](../world/index.md)·[WxDialogue](../dialogue/index.md), 레이어/액션 처리는 [WxUI](../ui/index.md)
- 상호작용 대상 탐지와 프롬프트 표시 — 계약만 여기 두고 스캔·사거리 판정은 소비 측에 있다
- 소환물의 정책 선언과 생성·수명·상한 집행 — 전부 [WxCombat](../combat/index.md)의 `UWxMinionComponent`·`UWxMinionSubsystem`에 있다. 여기 있는 것은 스폰 대상 마커와 주인 태그 이름뿐이다
...
| `IWxSpawnable` | 다른 주체가 스폰해 태어나는 액터의 계약. 처치 판정은 액터가 하고 `GetOnKilledDelegate()` 통지로 스폰 주체에 알린다. 스폰 주체를 아는 픽커([WxWorld](../world/index.md) 스포너, [WxCombat](../combat/index.md) 소환 노티파이)가 `MustImplement`로 이 계약을 강제한다 | [Plugins/WxCore/Source/WxCore/Public/WxSpawnable.h](../../../../Plugins/WxCore/Source/WxCore/Public/WxSpawnable.h) |
...
- 모든 네이티브 태그를 이 두 파일에만 선언·정의한다. 2026-09-21 전역 검색 기준 예외는 없다.
- 도메인 플러그인은 자체 태그 네임스페이스를 두지 않으며, 한 모듈 안에서만 쓰는 태그도 `WxGameplayTags`에 둔다. 헤더의 단일 선언·정의 규약은 예외 없이 적용한다(2026-09-21 사용자 확정).
- 주요 네임스페이스:
...
## 확장 포인트 / 규약
- **새 태그**: 헤더에 `UE_DECLARE_GAMEPLAY_TAG_EXTERN` + `WXCORE_API`, cpp에 `UE_DEFINE_GAMEPLAY_TAG` 한 쌍. 도메인 안에서만 쓰는 태그도 예외 없이 여기에 모은다.
...
## 관련 모듈
- 상위: 모든 Wx 플러그인과 [WxGame](../game/index.md)이 이 모듈을 참조한다. 태그 소비가 가장 두꺼운 곳은 [WxCombat](../combat/index.md)이고, `IWxInteractable` 구현은 [WxWorld](../world/index.md)·[WxDialogue](../dialogue/index.md)·[WxInventory](../inventory/index.md)·[WxGame](../game/index.md)에, `FWxLocatorUtils` 사용은 [WxWorld](../world/index.md)·[WxQuest](../quests/index.md)·[WxUI](../ui/index.md)의 StateTree 태스크에 있다
...
*이관 원문의 기준 커밋 `047197a` · 생성일 2026-09-16 · 소스 13파일 — 원문 출처 보존; 현재 확인 범위는 이 참고 정보 참고*
