...
WxInventory는 서버 권위로 아이템의 소유·획득·사용·충전을 처리하고 인벤토리를 복제합니다.
...
**경계 (비담당)**
- 상호작용 스캔·프롬프트 표시는 [WxWorld](../world/index.md)의 `UWxInteractionScannerComponent`가 담당하며, 픽업은 [WxCore](../foundation/index.md)의 `IWxInteractable` 계약만 구현한다(그래서 WxWorld를 참조하지 않는다)
...
- **사용 흐름**: UI/입력 → (`RequestUseConsumable`이 `Ability.UseItem` 태그로 GA 발동) → 몽타주의 `UWxAnimNotify_UseItem`이 `Event.UseItem` 발행 → `UWxItemUseComponent`가 서버에서 수신 → `UWxInventoryComponent::UseItemByDef`. 태그는 이 모듈이 선언하지 않고 [WxCore](../foundation/index.md)의 `WxGameplayTags.h`에서 가져온다.
- **데이터 주도 설정**: 보상은 `FWxRewardTableRow` DataTable Row(항목 최대 5, 아이템은 소프트 참조 → 지급 시점 로드), 시작 지급은 컴포넌트의 `StartingItems`가 정한다. StateTree의 `GiveRewards`·`RefillItemCharges`는 `Transition.SourceStateID`가 유효하지 않은 초기 진입과 비권위 실행을 건너뛴다. 모든 복원·재진입의 중복 방지를 보장하는 것은 아니므로 실제 트리 경로를 확인한다.
...
## 관련 모듈
- 상위: `WxGame`이 PlayerController에 인벤토리를 붙이고, UseItem 어빌리티와 MVVM 뷰모델([Source/WxGame/MVVM/](../../../../Source/WxGame/MVVM))이 이 모듈의 API·델리게이트를 소비한다. 획득은 [WxWorld](../world/index.md)(상호작용 스캔)와 StateTree 보상/리필 태스크에서 들어온다. 공용 계약(`IWxInteractable`, `WxGameplayTags`)은 [WxCore](../foundation/index.md).
...
[Build.cs](../../../../Plugins/WxInventory/Source/WxInventory/WxInventory.Build.cs)·[descriptor](../../../../Plugins/WxInventory/WxInventory.uplugin), InventoryComponent·ItemDefinition·ItemFragment·RewardTableRow의 공개 계약과 [인벤토리 전체 구현](../../../../Plugins/WxInventory/Source/WxInventory/Private/Inventory/WxInventoryComponent.cpp)을 확인했다.

- [ItemInstance](../../../../Plugins/WxInventory/Source/WxInventory/Private/Items/WxItemInstance.cpp): 정의·충전 복제와 컨트롤러 인벤토리로의 OnRep 통지.
- [ItemUseComponent](../../../../Plugins/WxInventory/Source/WxInventory/Private/Inventory/WxItemUseComponent.cpp): 서버에서 노티파이·메시 오너를 확인하고 예약 아이템을 비운 뒤 사용한다. `UseItemByDef`는 필요한 GE 스펙을 만든 다음 충전/스택을 먼저 차감하고 효과를 적용한다.
- [RewardLibrary](../../../../Plugins/WxInventory/Source/WxInventory/Private/WxRewardLibrary.cpp): Pickup Fragment가 없을 때 직접 지급하고, 있으면 지정 픽업을 지연 스폰한다. 픽업 클래스가 누락된 경우 직접 지급으로 대체하지 않고 경고 후 건너뛴다.
- [GiveRewards](../../../../Plugins/WxInventory/Source/WxInventory/Private/Inventory/WxStateTreeTask_GiveRewards.cpp)·[RefillItemCharges](../../../../Plugins/WxInventory/Source/WxInventory/Private/Inventory/WxStateTreeTask_RefillItemCharges.cpp): 0번 PlayerController를 대상으로 하는 현재 구현을 확인했다. 원격 플레이어별 보상 귀속 정책의 완성으로 해석하지 않는다.

아이템 에셋 조합, 실제 복제 순서와 픽업 시각 효과는 실행 검증하지 않았다.

*이관 원문의 기준 커밋 `047197a` · 생성일 2026-09-16 · 소스 24파일 — 원문 출처 보존; 현재 확인 범위는 이 참고 정보 참고*
