...
WxGame은 캐릭터·컨트롤러·게임 흐름에 각 도메인 기능을 조립하고 서로 연결합니다.
...
**경계 (비담당)**
- 전투 규칙·대미지·락온·히트스톱·어빌리티 베이스 → [WxCombat](../combat/index.md). 이 모듈의 어빌리티는 `UWxAbilityBase`를 상속만 한다.
- 아이템 정의·인벤토리 저장·보상 테이블 → [WxInventory](../inventory/index.md).
- 위젯·HUD 레이아웃·뷰모델 베이스(`UWxViewModel`) → [WxUI](../ui/index.md). 여기 있는 뷰모델은 WxUI가 볼 수 없는 도메인을 물기 위한 파생일 뿐이다.
- 상호작용 대상·스캐너·스포너·체크포인트 → [WxWorld](../world/index.md).
- 인지·행동트리 서비스·AI 행동 설정 → [WxAI](../ai/index.md). `AWxAIController`는 감각의 모양만 잡고 수치는 폰의 `UWxAIBehaviorComponent`가 밀어 넣는다.
- 대화 데이터·진행(`AWxDialogueActor`, `UWxDialogueSessionComponent`) → [WxDialogue](../dialogue/index.md), 퀘스트 저널·목표 판정 → [WxQuest](../quests/index.md).
- Native Gameplay Tag 선언과 공용 정의(`WxGameplayTags`, `IWxUIData`) → [WxCore](../foundation/index.md). 이 모듈은 태그를 선언하지 않고 참조만 한다.
...
## 관련 모듈
- 하위(위임 대상): [WxCore](../foundation/index.md) · [WxCombat](../combat/index.md) · [WxInventory](../inventory/index.md) · [WxUI](../ui/index.md) · [WxWorld](../world/index.md) · [WxAI](../ai/index.md) · [WxDialogue](../dialogue/index.md) · [WxQuest](../quests/index.md)
