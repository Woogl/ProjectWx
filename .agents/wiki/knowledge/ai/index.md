...
WxAI는 퍼셉션과 Blackboard로 타겟을 정하고, Behavior Tree로 AI의 이동·전투·소환수 행동을 구성합니다.
...
- 락온 "대상 보관"과 어트리뷰트/GameplayEffect 정의는 [WxCombat](../combat/index.md). WxAI는 이 모듈을 참조하지 않으므로, 어트리뷰트·감속 이펙트는 C++ 하드코딩 대신 BT 에디터에서 디자이너가 지정하는 방식으로 연결한다.
- 공용 GameplayTag 정의는 [WxCore](../foundation/index.md)(`WxGameplayTags`). 이 모듈은 태그를 선언하지 않고 소비만 한다.
...
- 함께 보는 모듈: [WxCore](../foundation/index.md) (GameplayTag 정의), [WxCombat](../combat/index.md) (락온 대상 보관·어트리뷰트·GameplayEffect — 코드 의존 없이 BT 저작으로만 연결)
...
*이관 원문의 기준 커밋 `2872e9a` · 생성일 2026-09-17 · 소스 41파일 — 원문 출처 보존; 현재 확인 범위는 이 참고 정보 참고*
