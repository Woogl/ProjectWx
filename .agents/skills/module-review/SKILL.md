...
2. 그 모듈의 `.agents/wiki/modules/<Name>.md`가 있으면 상태와 검증 범위를 확인하고 **먼저** 읽어 진입점·경계를 빠르게 잡는다(readme-writer 산출물, 없으면 건너뜀). 그 지도를 따라 `*.Build.cs`·`Public/`·`Private/` 헤더와 **핵심 로직(cpp)까지** 내려가 리뷰한다. 헤더만 훑지 말고 복잡·위험한 파일을 깊게 본다. **모든 파일을 통독하지 않는다** — 리뷰 가치가 높은 곳에 시간을 쓴다.
