...
하네스가 파일시스템 샌드박스 안에서 실행 중이면(Codex 데스크톱의 읽기 전용 샌드박스, Claude Code의 제한 모드 등) 첫 실행부터 샌드박스 밖 실행 승인을 요청한다. UnrealBuildTool이 `%LOCALAPPDATA%\UnrealBuildTool\Log.txt`와 `Trace.uba`를 기록해야 하기 때문이다. 샌드박스 안에서 먼저 실패시킨 뒤 재시도하지 않는다. 권한이 없으면 스크립트의 쓰기 사전 검사가 UBT 실행 전에 종료 코드 `2`로 중단한다.
