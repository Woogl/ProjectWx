...
상태: 확인 대기 · 후속 수정 빌드 확인
다음 행동: 0 피해 히트스톱과 투사체 되돌림을 플레이로 확인한다.

- 이전 상태: 1단계 사용자 플레이 확인 · 2단계 자체 검증/피니셔 수정 플레이 확인 · 3단계 단순화 회귀 통과 · 4단계 Hit 함수 분리 통과 · 네 인자 ApplyDamage 복원 빌드/회귀 통과
...
| 항목 | 확인 방법 | 담당 | 결과 | 근거 |
| --- | --- | --- | --- | --- |
| 0 피해 히트스톱 | 게임: 완전 경감 가드·반올림 0 피해에는 히트스톱이 없고 퍼펙트 가드에는 있다 | 사람 | 대기 |  |
| 히트스톱 체감 | 게임: 무기·투사체 명중의 히트스톱이 자연스럽다 | 사람 | 대기 |  |
| 퍼펙트 가드 투사체 되돌림 | 게임: 반사 가능한 투사체를 퍼펙트 가드하면 되돌아간다 | 사람 | 대기 |  |
| 반사 불가 투사체 파괴 | 게임: 반사 불가 투사체는 퍼펙트 가드 때 파괴된다 | 사람 | 대기 |  |
