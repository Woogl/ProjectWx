...
- 남은 일: 사용자 확인 → 커밋·푸시, 완료 때 Wiki 반영(값이 있는 곳: 규칙은 타입 C++, 콘텐츠는 GA_, 시점 값은 몽타주 노티파이, AI 선택은 BT 태그)과 기획 브리핑 수정 제안.
