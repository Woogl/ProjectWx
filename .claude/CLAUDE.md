...
4. **검증**: WxEditor(Development) 타겟을 빌드해 컴파일을 확인한다. 에디터 재실행은 불필요, 실패 시 `build-doctor`.

5. **완료 기록**: 검증 후, 같은 파일의 「완료」를 채운다. 이 단계는 별도 승인 없이 진행한다(승인 게이트는 1단계 plan 뿐).

예외: 동작·구조를 바꾸지 않는 사소한 작업(오타·주석·로그 문구·포매팅 등)은 절차 없이 진행할 수 있다. 동작·구조를 바꾸는 작업은 비사소로 보아 절차를 따르며, 애매하면 절차를 택한다.
...
| `WxAudio`    | 도메인            | 오디오/음악 (Chooser 기반 BGM)    |
| `WxAI`       | 도메인            | AI 시스템                         |
| `WxQuest`    | 도메인            | 퀘스트 시스템                     |
