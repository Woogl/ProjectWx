...
4. **작업 완료 후 문서화**: 코드 작성·검증을 마친 뒤, `.claude/works/`에 실제 수행한 작업 내용을 정리해 기록한다.
   * 파일명: `YYYY-MM-DD-제목.md` (대응하는 plan이 있으면 같은 제목을 사용한다.)
   * 구성: 작업 개요 / 변경한 파일·모듈 / 구현·결정 사항과 그 이유 / plan 대비 달라진 점 / 후속 과제.
   * plan이 "착수 전 의도"라면 works는 "완료 후 실제 결과"를 남기는 기록이다.

예외: 읽기 전용 작업(질의·탐색·분석)과 명백히 사소한 단일 수정(오타·주석·로그 문구 등)은 위 절차(plan·works 문서화 포함) 없이 진행할 수 있다. 판단이 애매하면 절차를 거친다.
