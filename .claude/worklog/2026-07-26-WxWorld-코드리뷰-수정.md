...
- **`Spawn Niagara` 는 자기 스폰 핸들로 판단**: 지속 FX(루프)와 트리거 FX 가 같은 노드를 쓰는데 `bShouldStateChangeOnReselect` 는 타입 단위라 둘을 못 가른다. 스폰한 컴포넌트를 인스턴스 데이터에 들고 살아 있으면 통과시키면 한 규칙으로 둘 다 맞는다 — 루프는 컴포넌트가 남아 중복이 막히고, 일회성은 `bAutoDestroy` 로 사라져 다음 발동에 다시 스폰된다.
...
### 후속 과제
- **에셋(사람 몫)**: `Content/WorldObject/Gimmick/ST_CheckPoint.uasset` 의 모닥불 `Spawn Niagara` 노드에서 `bPlayOnRestore = true` 를 켠다. 복원 게이트가 생겼으므로 켜지 않으면 로드 후 모닥불 불꽃이 사라진다.
- **런타임 검증(미수행)**: ① 모닥불 FX 가 복원·재선택에서 겹치지 않는지, ② 같은 층 엘리베이터 호출이 전 피어에서 조용한지, ③ 이동 중인 문/플랫폼이 재선택에 끊기지 않는지, ④ 상태 전이 중에도 켜져 있는 영역(체크포인트 Unlit→Lit)의 HUD 문구가 영역 안에 선 채로 즉시 갱신되는지, ⑤ 스포너 attach 제거 후 스폰 AI 의 원격 이동 스무딩, ⑥ 체크포인트 재휴식에서 리필·저장이 빠지는 변화가 기획 의도에 맞는지.
