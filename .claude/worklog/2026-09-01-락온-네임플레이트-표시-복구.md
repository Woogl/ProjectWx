...
| `Plugins/WxCombat/.../Private/Targeting/WxLockOnPointComponent.cpp` | 값이 바뀔 때만 방송하는 세터 구현 | 수정 |
...
| 파일 | 수정한 내용 | 구분 |
|---|---|---|
| `Plugins/WxCombat/.../Public/Targeting/WxLockOnPointComponent.h` | `FWxOnLockedOnChanged` 델리게이트, `SetLockedOn()`·`IsLockedOn()`, `bLockedOn` 추가 | 수정 |
| `Plugins/WxCombat/.../Private/Targeting/WxLockOnPointComponent.cpp` | 값이 바뀔 때만 방송하는 세터 구현 | 수정 |
...
- 부위별 락온으로 지점이 여러 개인 적이 생기면 적의 구독을 `GetComponents` 순회로 넓혀야 한다. 지금은 지점이 하나뿐이다.
