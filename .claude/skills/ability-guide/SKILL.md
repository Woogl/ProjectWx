...
게임 코드를 분석하여 어빌리티 가이드 HTML 문서를 `Docs/ability-guide.html`에 생성하라.
...
6. **어빌리티 상호작용 다이어그램**:
   - 어빌리티 간의 관계를 시각적으로 보여주는 다이어그램 (순수 HTML+CSS로 구현)
   - 예: 공격 → 피격 반응 유발, DP 누적 → 그로기 발동, HP 0 → 사망 발동
   - 가드/무적이 피격 반응을 어떻게 변경하는지
...
`Docs/ability-guide.html`에 생성하라. 기존 파일이 있으면 덮어쓴다.
