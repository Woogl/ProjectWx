...
- 부활 위치를 실제 액터로 해석하는 로직 + 태그 갱신 시점: GameMode 의 `ChoosePlayerStart` 가 `GetPlayerStartTag()` 로 받은 Tag 를 `FindPlayerStart` 에 넘기고, 선택된 PlayerStart 태그를 `SetPlayerStartTag` 로 되기록한다(WxGame 측 `AWxGameMode`).
- 체크포인트 액터/상호작용 자체: 호출 측이 `SetPlayerStartTag` 호출 + `SaveSlot` 영속을 수행한다(WxGame 측 `AWxCheckPoint`).
...
| `UWxSaveGame` | 슬롯 데이터 컨테이너. `TMap<FGuid, FWxActorRecord>` + `PlayerStartTag` 보관 | `Plugins/WxSave/Source/WxSave/Public/WxSaveGame.h` |
