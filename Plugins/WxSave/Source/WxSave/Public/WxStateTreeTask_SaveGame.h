...
/**
 * 라이브 전이로 진입할 때 권위 측에서만 다음 월드 틱에 라이브 상태를 활성 슬롯에 플러시·기록하고 Succeeded 로 완료한다(체크포인트 오토세이브).
 * 한 틱 미루는 이유는 StateTree 컴포넌트가 Super 틱 뒤에 새 활성 상태를 StateTag 로 발행하므로, 진입 태스크 안에서 즉시 저장하면 이전 상태가 기록되기 때문이다.
