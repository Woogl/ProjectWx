...
## 책임
**담당**
- 대화 대상 호스트 및 상호작용 진입점 (`AWxDialogueActor`가 `IWxInteractable` 구현, 대화 정의는 `UWxDialogueComponent`)
...
| 타입 | 역할 | 위치 |
| --- | --- | --- |
| `AWxDialogueActor` | 말 걸 수 있는 대상의 공통 호스트(Abstract). `IWxInteractable` 구현, 대화 컴포넌트로 위임 | `Plugins/WxDialogue/Source/WxDialogue/Public/WxDialogueActor.h` |
| `UWxDialogueComponent` | 대화 정의(시작 행·표시 이름)와 세션 진입. 계약은 들지 않는다 | `Plugins/WxDialogue/Source/WxDialogue/Public/WxDialogueComponent.h` |
...
- **새 대화 대상**: `AWxDialogueActor`를 상속한 BP를 만들고(몸통 메시는 BP가 세운다) 대화 컴포넌트의 `StartRow`(시작 행 핸들)·`SpeakerName`을 지정. 사람 형태면 `AWxNpc`(WxGame)를 상속한다. 컴포넌트만 아무 액터에 붙여서는 대상이 되지 않는다 — 계약은 액터가 든다.
- **말 걸기 잠금**: 액터의 `bInteractionEnabled`가 활성 상태다(기본 켜짐). 퀘스트가 탑재되기 전까지 말을 걸 수 없어야 하면 BP·배치 인스턴스에서 미리 끄고, '상호작용 켜기' ST 태스크가 계약으로 연다. 감지·사거리는 몸통 형상이 재므로 이 값과 무관하다.
...
2. `Plugins/WxDialogue/Source/WxDialogue/Private/WxDialogueComponent.cpp` — 상호작용→세션 시작으로 넘어가는 짧고 명확한 진입 흐름(`StartDialogueWith`)
