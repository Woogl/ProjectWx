...
## 책임
**담당**
- 대화 데이터 모델: 노드 = 행, 대화 1편 = 테이블 1개 (`FWxDialogueTableRow` / `FWxDialogueLine` / `FWxDialogueChoice`)
- 세션 진행: 라인 넘기기·선택지 분기·선형 진행·종료를 PlayerController 측 세션이 소유 (`UWxDialogueSessionComponent`)
...
| `FWxDialogueTableRow` | 대화 노드 데이터. Lines→Choices/NextRow 로 그래프 구성 | `Source/WxDialogue/Public/WxDialogueTableRow.h` |
...
## 확장 포인트 / 규약
- **새 대화**: `FWxDialogueTableRow` 로우 타입 DataTable 을 만들고, 액터의 `UWxDialogueComponent::StartRow` 에 시작 노드를 지정한다. 분기는 `Choices[].TargetRow`, 선형 진행은 `NextRow`(None 이면 종료).
...
2. `Source/WxDialogue/Public/WxDialogueTableRow.h` — 대화가 어떤 그래프로 표현되는지(노드=행)를 먼저 잡아야 세션 순회가 읽힌다.
