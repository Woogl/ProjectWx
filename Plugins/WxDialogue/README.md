...
> 말을 걸 수 있는 대상과 나눈 대화 한 편을 데이터 테이블로 정의하고, 대사를 한 줄씩 넘기며 카메라·포즈 연출과 함께 재생한다. 대화의 의미(퀘스트 수주 등)는 해석하지 않는다.
...
- 대화가 뜻하는 바(퀘스트 진행 등): 종료를 기다린 트리([[WxQuest]] 등)가 판정
...
## 관련
- 상위: [[WxQuest]]·[[WxAI]] StateTree가 Play Dialogue 태스크로 대사를 낸다. 세션 상태는 [[WxUI]](대화 창)가 읽는다.
