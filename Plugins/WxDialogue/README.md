...
> NPC·기믹과 플레이어 사이의 분기형 대화를 진행한다. DataTable 로 정의한 대화 그래프를 노드 단위로 순회하며, 표시(UI)와 의미 해석(퀘스트)은 다른 도메인에 넘긴다.

## 책임
**담당**
- 대화 데이터 모델: 노드 = 대사 한 줄 = 행, 대화 1편 = 테이블 1개 (`FWxDialogueTableRow` / `FWxDialogueChoice`)
- 세션 진행: 대사 넘기기·선택지 분기·선형 진행·종료를 PlayerController 측 세션이 소유 (`UWxDialogueSessionComponent`)
...
| 타입 | 역할 | 위치 |
| --- | --- | --- |
| `UWxDialogueSessionComponent` | PC 소유 세션. Start/Advance/Choose 로 진행을 몰고 델리게이트로 발행 | `Source/WxDialogue/Public/WxDialogueSessionComponent.h` |
...
| `FWxDialogueTableRow` | 대화 노드 데이터. 대사 한 줄(Speaker/Line) + Choices/NextDialogue 로 그래프 구성 | `Source/WxDialogue/Public/WxDialogueTableRow.h` |
...
## 확장 포인트 / 규약
- **새 대화**: `FWxDialogueTableRow` 로우 타입 DataTable 을 만들고, 액터의 `UWxDialogueComponent::StartRow` 에 시작 노드를 지정한다. 분기는 `Choices[].TargetDialogue`, 선형 진행은 `NextDialogue`(None 이면 종료).
...
2. `Source/WxDialogue/Public/WxDialogueTableRow.h` — 대화가 어떤 그래프로 표현되는지(노드=대사 한 줄=행)를 먼저 잡아야 세션 순회가 읽힌다.
3. `Source/WxDialogue/Private/WxDialogueSessionComponent.cpp` — EnterRow/Advance/Choose 실제 순회와 `State.Dialogue` 태그 부착/해제.
