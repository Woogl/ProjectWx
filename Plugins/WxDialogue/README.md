...
| `FWxStateTreeTask_WaitDialogueCompleted` | 지정 대화의 완주를 관찰(게이트) | `Plugins/WxDialogue/Source/WxDialogue/Public/WxDialogueStateTreeNodes.h` |
| `FWxStateTreeTask_PlayDialogue` | 트리가 대사를 열어 연출 | `Plugins/WxDialogue/Source/WxDialogue/Public/WxDialogueStateTreeNodes.h` |
| `FWxStateTreeTask_EnableNpcInteraction` | 지정 NPC들의 상호작용 토글 | `Plugins/WxDialogue/Source/WxDialogue/Public/WxDialogueStateTreeNodes.h` |
...
- **퀘스트/StateTree에서 사용**: 소비 도메인은 대화 모듈을 참조하지 않고 에셋에서 세 태스크를 골라 쓴다(대화 노드를 이 모듈이 함께 제공). NPC 지목은 `FUniversalObjectLocator` 배열.
...
4. `Plugins/WxDialogue/Source/WxDialogue/Public/WxDialogueStateTreeNodes.h` — 퀘스트 등 소비 도메인이 대화를 관찰·출력·토글하는 세 태스크.
