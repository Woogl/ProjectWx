...
- 대화 상대 컴포넌트(`UWxDialogueComponent`, 대상 액터에 부착, 시작 행·화자 이름 보유 + `IWxInteractable` 구현) 및 세션 진행(`UWxDialogueSessionComponent`, PlayerController에 주입)
- 대화 연출: 전용 카메라 구도·전환, 대사별 대상 포즈(AnimMontage) 비동기 스트리밍·재생, 세션 중 폰 ASC의 `State.Dialogue` 태그 부여
- 상호작용 영역 콜리전 토글(잠금·해제)
...
| `UWxDialogueComponent` | 대상 액터를 대화 상대로 만드는 컴포넌트. 상호작용 계약을 들고 시작 행을 세션에 넘긴다 | `Plugins/WxDialogue/Source/WxDialogue/Public/WxDialogueComponent.h` |
...
- **새 대화 상대 추가**: 액터에 `UWxDialogueComponent`를 붙이고 `StartRow`·`SpeakerName`·`AreaMesh`(상호작용 영역이 될 메시)를 지정하면 끝 — 전용 액터 클래스가 필요 없다(기믹 컴포넌트와 같은 갈래). C++ 호스트는 생성자에서 `SetAreaMesh`로 넘긴다(예: `AWxNpc`, WxGame). 잠긴 채 시작할 대상은 별도 플래그 없이 영역 메시 콜리전을 배치 인스턴스에서 미리 꺼 둔다.
