 ...
 * 덕분에 대화 상대를 다루는 코드('Enable Npc Interaction' 태스크)가 호스트 액터 타입을 몰라도 되고, 호스트를 어느 모듈에 두든 무방하다.
...
public:
	...
	/**
	 * 이 대상에게 말을 걸 수 있는지를 영역 메시의 쿼리 콜리전으로 토글한다. 'Enable Npc Interaction' 태스크가 퀘스트 단계에 맞춰 호출한다.
