...
/**
 * 대화 정의 컴포넌트. 어느 노드에서 대화를 시작할지만 보유하고, 세션 진행은 상호작용한 플레이어 측(UWxDialogueSessionComponent)이 소유한다.
 *
 * 상호작용 계약은 호스트 액터(AWxDialogueActor)가 들고 이 컴포넌트로 넘긴다 — 계약은 액터 전용이라 컴포넌트가 구현하지 않는다.
 * 그래서 이 컴포넌트를 아무 액터에 붙여도 말을 걸 수 있게 되지는 않는다. 대화 상대는 AWxDialogueActor 를 상속해 만든다.
 */
...
public:
	...
	/** HUD 프롬프트 문구. 호스트 액터가 계약으로 답할 때 읽는다. */
