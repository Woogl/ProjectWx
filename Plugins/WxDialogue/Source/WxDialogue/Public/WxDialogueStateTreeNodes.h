 ...
 * 목격은 진행 중인 대사와 지정 행을 맞춰 보고, 완주는 그 뒤 대화창이 닫히는 것으로 본다 — 지정 행이 시작 행이면 "그 대화를 완주했는가"가 되고, 중간·끝 행이면 "그 대사까지 읽고 끝냈는가"가 된다.
...
struct FWxStateTreeTask_EnableNpcInteractionInstanceData
{
	...
	/** 진입 시 그 NPC 들의 상호작용 활성 여부. 기본이 잠금인 것은 이 태스크를 놓는 이유가 대개 그것이기 때문이다. */
