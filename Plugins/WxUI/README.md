...
- **새 인디케이터**: WBP는 `UWxIndicatorWidget`(Abstract)을 상속해 `OnIndicatorRefreshed`로 외형 저작(위치는 캔버스가 정하므로 위젯은 모른다). 매니저 부착은 GameMode의 `FrameworkComponents` 주입으로, StateTree에서는 `FWxStateTreeTask_MarkIndicator` 노드로 대상 위에 표시.
