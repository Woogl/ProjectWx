...
## 확장 포인트 / 규약
- **새 화면 위젯**: `UWxActivatableWidget` 을 상속(BP)하고, `UWxUIDeveloperSettings`·`UWxHUDLayout` 의 소프트 클래스 슬롯에 지정하거나 `UWxUIManagerSubsystem::PushContentToLayer`·`UWxUILibrary`·`UWxAsyncAction_PushWidgetToLayer` 로 레이어에 push 한다. 레이어는 `UI.Layer.*` 게임플레이 태그로 식별(네이티브 선언 아님, 문자열 태그). z-order/레이어 목록은 `UWxPrimaryGameLayout::LayerTags` 배열로 저작.
...
- **도메인 값 주입 규약**: WxUI 는 도메인 타입을 참조하지 않으므로, VM 은 엔진 타입/평면 표시 필드만 노출하고 값은 외부 소스가 push 한다(`SetSelection`, `Initialize` 등). 네임플레이트는 `UWxNameplateComponent::InitializeViewModels(ASC, FWxCharacterUIData)` 로 바인딩.
- **아이콘/초상화**: `TSoftObjectPtr<UTexture2D>` 를 그대로 노출만 하고 View 측 `UWxLazyImage`(1-arg MVVM setter)/`UCommonLazyImage` 가 비동기 로드·수명 관리(VM 은 `LoadSynchronous` 미호출).
...
---
*문서 기준 커밋 `94f2eaf` · 생성일 2026-07-20 · 소스 56파일 — `/readme-writer`로 갱신*
