...
- 화면 좌표 인디케이터·네임플레이트: `UWxIndicatorManagerComponent`(로컬 PC 투영), `UWxNameplateComponent`(ASC 태그 기반 표시/거리 스케일).
- 공용 위젯 베이스: `UWxActivatableWidget`·`UWxButtonBase`·`UWxTabListWidgetBase`·`UWxGamePopup` 등 WBP가 파생할 C++ 베이스와 BP 파사드(`UWxUILibrary`).
...
- 규칙: 「WxCore 외 Wx 플러그인 참조」 없음 ✅ (`WxUI.Build.cs` 의존은 엔진 모듈 + `WxCore`뿐)
...
- 레이어 태그(`UI.Layer.*`)는 네이티브 C++ 선언이 아니라 태그 소스에 정의되며, `UWxPrimaryGameLayout::LayerTags`의 배열 순서가 z-order다(0 = 최하단).
...
- StateTree 노드(`WxIndicatorStateTreeNodes`·`WxSubtitleStateTreeNodes`)로 인디케이터/자막을 태스크 단위로 붙인다.
...
4. `Source/WxUI/Public/WxUILibrary.h` — BP에서 UI를 다루는 파사드 표면.
...
---
*문서 기준 커밋 `28ee2c6` · 생성일 2026-08-03 · 소스 63파일 — `/readme-writer`로 갱신*
