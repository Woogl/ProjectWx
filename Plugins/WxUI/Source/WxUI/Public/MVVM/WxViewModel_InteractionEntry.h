...
#include "WxViewModel_InteractionEntry.generated.h"

/**
 * 상호작용 리스트의 한 항목 뷰모델.
 * UWxViewModel_Interaction 이 인-레인지 대상 하나당 하나씩 생성/소유한다.
 * ListView 엔트리 위젯이 Prompt(표시 텍스트)와 bSelected(현재 선택 여부)에 바인딩한다.
 */
UCLASS()
class WXUI_API UWxViewModel_InteractionEntry : public UWxViewModel
{
	GENERATED_BODY()
