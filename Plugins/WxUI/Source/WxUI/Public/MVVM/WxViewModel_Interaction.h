...
class UWxViewModel_InteractionEntry;

/**
 * 상호작용 HUD 리스트 뷰모델.
 * 현재 범위 안에 있는 상호작용 대상들을 엔트리 VM 목록으로 노출하고, 그중 선택된 인덱스를 표시한다.
 *
 * WxUI 는 WxWorld(선택자)를 참조할 수 없으므로, 목록/선택 변경은 WxGame 리졸버가 선택자 델리게이트를
...
public:
	/** 초기 목록/선택으로 엔트리를 구성하고 초기화 완료로 표시한다. */
	...
	/** 상호작용 대상 엔트리 목록. ListView 가 본 프로퍼티에 바인딩한다. */
	UPROPERTY(BlueprintReadOnly, FieldNotify, Category = "Wx|Interaction")
	TArray<TObjectPtr<UWxViewModel_InteractionEntry>> Entries;

	/** 현재 선택된 엔트리 인덱스(없으면 INDEX_NONE). */
	UPROPERTY(BlueprintReadOnly, FieldNotify, Category = "Wx|Interaction")
	int32 SelectedIndex = INDEX_NONE;

private:
	/** 프롬프트 목록으로 엔트리 VM 들을 재구성한다. */
	void RebuildEntries(const TArray<FText>& InPrompts);

	/** 선택 인덱스를 클램프해 각 엔트리의 bSelected 와 SelectedIndex 를 갱신한다. */
