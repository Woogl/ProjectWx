 ...
 *
 * UWxItemDefinition.Fragments 에 FInstancedStruct 형태로 부착되어, 아이템에
 * 데이터 단위 행동/속성을 컴포지션한다. 가상 함수는 사용하지 않으며, 처리
 * 로직은 외부 시스템(예: 인벤토리/장비 매니저)이 타입별로 디스패치한다.
 */
 ...
 * 매니저는 장착 시점에 본 클래스를 스폰하여 소유 캐릭터에 attach 하고, 해제
 * 시점에 destroy 한다.
 */
