...
| 타입 | 역할 | 위치 |
| --- | --- | --- |
| `UWxInventoryManagerComponent` | 인벤토리 매니저 컴포넌트. 추가/차감/사용/장착의 권한 진입점, 변경 델리게이트 보유 | `Plugins/WxInventory/Source/WxInventory/Public/Inventory/WxInventoryManagerComponent.h` |
| `FWxInventoryList` | 엔트리 컬렉션. FastArray 레플리케이션 + 머지/분할/차감 로직 | 동상 |
| `FWxInventoryEntry` | 인벤토리 한 슬롯(인스턴스 + StackCount) | 동상 |
...
| `UWxItemInstance` | 아이템 런타임 인스턴스. 정의 바인딩 + 충전량 보유, GE SourceObject | `Plugins/WxInventory/Source/WxInventory/Public/Items/WxItemInstance.h` |
| `UWxItemFragment` | Fragment 베이스(EditInline). `OnInstanceCreated`로 인스턴스 초기화 | `Plugins/WxInventory/Source/WxInventory/Public/Items/WxItemFragment.h` |
| `UWxItemFragment_Stackable` | 스택 가능 선언(`MaxStack`). 부재 시 1슬롯=1개 강제 | 동상 |
| `UWxItemFragment_Usable` | 사용 시 적용할 GameplayEffect | 동상 |
| `UWxItemFragment_Charges` | 인스턴스 단위 충전(에스트병 방식, `MaxCharges`) | 동상 |
| `UWxItemFragment_Equippable` | 장착 시 메시 스왑/소켓/장착 GE | 동상 |
...
| `FWxRewardTableRow` | 보상(아이템·수량 Pair 최대 5개) 데이터테이블 Row | `Plugins/WxInventory/Source/WxInventory/Public/Items/WxRewardTableRow.h` |
...
- **새 행동 추가**: `UWxItemFragment`를 상속하고 필요 시 `OnInstanceCreated`를 override해 인스턴스 초기 상태를 주입한다(예: Charges가 MaxCharges로 채움).
- **스택 규칙**: `Stackable` Fragment의 `MaxStack`까지 기존 엔트리에 머지하고 초과분은 새 엔트리로 분할. Fragment 부재 시 항상 1슬롯=1개. 같은 정의가 여러 슬롯에 분산돼도 `GetTotalItemCountByDefinition`/`ConsumeItemsByDefinition`로 합산 조회·차감한다.
- **충전형 vs 소모형**: Charges Fragment가 있으면 사용 시 인벤토리 스택은 유지하고 인스턴스 충전량만 1 감소(소진돼도 인벤토리에 남음), 없으면 스택 1 차감. 회복은 `RefillItemCharges`(체크포인트 리필).
- **권한/리플리케이션(최대 4인 멀티)**: 인벤토리는 PlayerController에 부착되며 `FindInventory`가 폰→컨트롤러를 거쳐 조회한다. Add/Consume/Use/Equip은 모두 서버(권한)에서만 호출되어야 한다(`check(HasAuthority())`). `FWxInventoryList`(FastArray) + `UWxItemInstance`(등록 SubObject)로 클라이언트 동기화하고, 서버 변경 경로와 클라이언트 복제 콜백(`PostReplicatedAdd/Change`, `PreReplicatedRemove`, `OnRep_CurrentCharges`)이 동일한 `Notify*` 진입점으로 수렴해 델리게이트를 발행한다.
...
## 관련
- 상위: `WxGame`(컨텐츠/픽업), [[WxUI]](인벤토리 표현), [[WxWorld]](픽업 액터/드롭), [[WxCombat]](장착 구현·GE), [[WxQuest]](보상 지급)

---
*문서 기준 커밋 `59bfe3f` · 생성일 2026-06-08 · 소스 12파일 — `/readme-writer`로 갱신*
