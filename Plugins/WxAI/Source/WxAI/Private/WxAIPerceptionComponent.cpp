...
void UWxAIPerceptionComponent::HandleTargetPerceptionUpdated(AActor* Actor, FAIStimulus Stimulus)
{
	...
	if (Stimulus.Type == UAISense::GetSenseID<UAISense_Hearing>())
	{
		...
		{
			WxAIBlackboardKeys::SetTargetLastKnownLocation(BB, Stimulus.StimulusLocation);
		}
		return;
	}
	...
	else if (WxAIBlackboardKeys::GetTargetActor(BB) == Actor)
	{
...
void UWxAIPerceptionComponent::SetTargetActor(AActor* NewTarget)
{
	...
	if (WxAIBlackboardKeys::GetTargetActor(BB) == NewTarget)
	{
		return;
	}

	WxAIBlackboardKeys::SetTargetActor(BB, NewTarget);
...
void UWxAIPerceptionComponent::UpdateRecognition()
{
	...
	const AActor* Target = WxAIBlackboardKeys::GetTargetActor(BB);
	if (!Target)
	{
	...
	const float PawnHomeDistSquared = FVector::DistSquared(Pawn->GetActorLocation(), WxAIBlackboardKeys::GetHomeLocation(BB));
	const bool bExceededLeash = PawnHomeDistSquared > LeashRadius * LeashRadius;
	if (bExceededLeash)
	{
		SetTargetActor(nullptr);
		WxAIBlackboardKeys::ClearTargetLastKnownLocation(BB);
		SetRecognized(false);
		return;
	}
