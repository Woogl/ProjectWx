 ...
 * 실패는 조건에 막힌 자식 하나를 엔진에 되돌려주는 방식으로 만든다 — 엔진이 그 자식의 조건을 다시 검사해 실패시키면서 탐색 결과를 Failed 로 세팅한다.
 * 따라서 조건은 전원 통과했는데 가중치만 전원 0 인 경우는 되돌려줄 자식이 없어 실패를 만들지 못하고, 부모는 직전 결과를 그대로 이어받는다.
 *
