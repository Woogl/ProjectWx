 ...
 * 단, 조건은 전원 통과했는데 가중치만 전원 0 이면 실패를 만들지 못해 부모는 직전 결과를 그대로 이어받는다.
 *
 * Selector 시멘틱과 다르다 — 일단 선택된 자식이 실행 후 실패해도 다른 자식으로 폴백하지 않고 그대로 실패를 반환한다.
 *
 * 조건 실패 자식에는 인덱스와 무관하게 전부 활성화 실패를 통지한다 — 무작위 추첨엔 자식 간 우선순위가 없어 앞뒤를 가를 기준이 없다.
...
protected:
	 ...
	 * 조건을 통과하지 못한 자식이 회피 완화로 되살아나는 일은 없다.
	 */
