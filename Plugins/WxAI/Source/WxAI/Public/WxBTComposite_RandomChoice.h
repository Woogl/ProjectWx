 ...
 *
 * 후보 수집 시 각 자식의 조건 Decorator(CompareAttributeRatio 등)를 평가해, 실행이 막힌 자식은 추첨에서 제외한다. 즉 "현재 유효한 후보들 중에서만" 가중 추첨한다. RandomWeight Decorator 는 조건이 아니므로 이 필터에 걸리지 않지만, 가중치가 0 인 자식은 뽑힐 수 없으므로 후보에서 제외한다.
 ...
 * 단 유효 후보가 1개뿐이면 회피는 무의미하므로 무시되고 그 자식이 다시 선택된다. 조건을 통과하지 못한 자식이 회피 완화로 되살아나는 일은 없다.
 *
...
protected:
	 ...
	 * 조건을 통과한 유효 후보가 1개뿐이면 회피는 무시되고 그 자식이 다시 선택된다.
	 */
