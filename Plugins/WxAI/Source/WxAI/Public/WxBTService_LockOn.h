 ...
 * AI 판 락온이며, 전투 브랜치의 최상위 노드에 얹어 쓴다. 플레이어의 락온(WxCombat 의 UWxLockOnComponent)과는 이름만 같고 별개의 구현이다.
 *
 * 포커스는 컨트롤러가, 회전 모드는 폰이 들고 있어 소유자가 갈라지기 쉬운 상태다. 이 서비스가 둘을 한 쌍으로 묶어 단독으로 소유한다 —
 * 퍼셉션은 TargetActor 발행까지만 맡으므로 두 시스템이 같은 상태를 두고 다투지 않는다.
 * Gameplay 우선순위 포커스도 이 노드만 쓴다는 전제라, 같은 우선순위를 쓰는 엔진 노드(RotateToFaceBBEntry 등)와 한 브랜치에 두지 않는다.
 *
 * 대상 소실은 Blackboard 관찰자가 아니라 틱 폴링으로 잡는다. 관찰자 알림이 브랜치를 실제로 접느냐는 게이트 데코레이터의
 * Observer aborts 설정에 달려 있어, 그 저작 값에 따라 해제가 늦어지기 때문이다. 브랜치를 접는 판정 자체는 BT 의 몫으로 남긴다.
 */
UCLASS()
