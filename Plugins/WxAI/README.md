...
> 적/NPC 폰의 행동을 구동하는 빌딩 블록을 제공한다. BehaviorTree 노드(Task/Service/Decorator/Composite), AI Perception, 정찰 경로 컴포넌트, Blackboard 키 규약, 팀 구분 타입을 한곳에 모은다.

## 책임
**담당**
- BehaviorTree 노드 제공 — GAS 어빌리티 발동, 정찰/배회 이동, 어트리뷰트 비율 조건, 무작위 분기 Composite, 타겟 거리 갱신 Service, 디버그 출력.
...
| `UWxBTService_TargetDistance` | Self↔Target 거리를 BB `TargetDistance`(Float)에 주기 기록 → 엔진 기본 arithmetic 데코레이터로 근/원거리 분기(커스텀 데코레이터 불필요) | `Plugins/WxAI/Source/WxAI/Public/WxBTService_TargetDistance.h` |
...
## 확장 포인트 / 규약
- 새 BT 노드 추가 — Task는 `UBTTaskNode`(또는 엔진 파생, 예: `UBTTask_MoveTo`), Service는 `UBTService`, Decorator는 `UBTDecorator`, Composite는 적절한 베이스(`UBTComposite_*`)를 상속하고 접두사 `WxBT...`를 따른다. Composite에서 자체 노드 메모리가 필요하면 `FWxBTRandomChoiceMemory`처럼 `FBTCompositeMemory` 뒤에 상태를 배치한다(엔진 메모리 레이아웃 보존). 폰별 독립 상태가 필요하면 `bCreateNodeInstance`(Patrol 참고).
...
- 새 Blackboard 키 추가 — `WxBlackboardKeys`에 `extern const FName`과 타입드 accessor를 함께 선언/정의해 타입 오용을 막는다. Object 키는 nullptr Set이 Clear와 동치이나, Vector/Float 키(예: `TargetLastKnownLocation`, `TargetDistance`)는 "값 없음"을 Set으로 표현할 수 없어 별도 Clear accessor를 둔다.
...
---
*문서 기준 커밋 `1a693b0` · 생성일 2026-07-02 · 소스 25파일 — `/readme-writer`로 갱신*
