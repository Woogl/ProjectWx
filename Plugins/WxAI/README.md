...
| `UWxBTDecorator_BeyondLeash` | 홈에서 LeashRadius 이탈 판정, 복귀 브랜치 게이팅(폴링 재평가) | `Plugins/WxAI/Source/WxAI/Public/WxBTDecorator_BeyondLeash.h` |
| `UWxBTTask_ReturnHome` | 리시 복귀 이동 + 퍼셉션 억제 진입/해제 | `Plugins/WxAI/Source/WxAI/Public/WxBTTask_ReturnHome.h` |
...
- **리시(leash) 모델**: `BeyondLeash`(FlowAbortMode=Lower Priority 고정, Self/Both 금지) + `ReturnHome`(억제 진입/해제)가 짝을 이룬다. 복귀 브랜치는 전투 브랜치보다 상위 우선순위에 배치해야 게이팅이 성립한다.
