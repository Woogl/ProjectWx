 ...
 *
 * WxInteractable: 상호작용 볼륨 표식용 Channel. UWxInteractionComponent 가 지정한 볼륨 메시의 응답을 Overlap 으로 켠다.
 *                 DefaultResponse=Ignore 이므로 응답을 켜지 않은 다른 메시엔 무영향이고, 다른 쿼리/트레이스에도 끼어들지 않는다.
 *                 플레이어 측 스캐너가 OverlapMultiByChannel(WxInteractable) 으로 주변 상호작용 볼륨을 수집한다.
 */
