 ...
 * 상호작용 영역은 액터의 메시 그 자체이며, 어느 메시가 영역인지는 대상이 IsInteractionMeshActive 로 직접 답한다.
...
public:
	...
	/**
	 * 어느 메시가 영역인가(표식)와 그 영역이 지금 켜져 있는가(활성)를 이 판정 하나가 함께 답한다.
	...
	/**
	 * 'Enable Interaction' 태스크가 액터를 지목했을 때 부른다.
	 ...
	 * Source 는 프롬프트를 물어보는 대상 메시다 — 영역이 여럿이면 영역별 문구를 가르고, 단일 영역이면 무시한다.
	 */
