...
- 도메인 간 결합을 끊기 위한 공용 인터페이스/추상 베이스 선언 (`IWxSavable`, `UWxAbilityComponent`)
...
- `UWxAbilityComponent`의 구체 구현체(UI 데이터 등)는 각 도메인 모듈이 상속해 정의
...
2. `Plugins/WxCore/Source/WxCore/Public/WxSavable.h` — 도메인 결합 분리 패턴의 대표 예시 (인터페이스 후크)
...
---
*문서 기준 커밋 `5ae4876` · 생성일 2026-06-11 · 소스 7파일 — `/readme-writer`로 갱신*
