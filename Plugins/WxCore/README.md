...
- 소환물의 정책 선언과 생성·수명·상한 집행 — 전부 [[WxCombat]]의 `UWxMinionComponent`·`UWxMinionSubsystem`에 있다. 여기 있는 것은 스폰 대상 마커와 주인 태그 이름뿐이다
...
| `IWxSpawnable` | 다른 주체가 스폰해 태어나는 액터의 계약. 스폰 주체를 아는 픽커([[WxWorld]] 스포너, [[WxCombat]] 소환 노티파이)가 `MustImplement`로 이 계약을 강제한다 | `Plugins/WxCore/Source/WxCore/Public/WxSpawnable.h` |
...
- **새 스폰 대상**: 액터가 `IWxSpawnable`을 구현한다. `OnSpawnedBy`는 Deferred Spawn의 `FinishSpawning` 이전에 불린다. 소환물 정책(상한·주인 태그)은 여기 없고 [[WxCombat]]의 `UWxMinionComponent`가 선언한다.
