...
## 책임
**담당**
- 상태→곡 선택: `FWxBGMChooserContext`(플레이어 ASC owned-tag + BGM 분류 태그 + 소스 소유자 태그)를 Chooser 테이블에 넣어 `UWxBGMData` 를 고른다. 우선순위 해소는 코드가 아닌 테이블 Row 순서(위→아래, 첫 매치 승)가 담당한다.
...
- 상태 기반 국지 BGM 소스(`UWxBGMSourceComponent`) 등록·집계와 소스 소진 시 베이스라인 자동 폴백.
- Blueprint 진입점(`StartBGM`/`StopBGM`)과 프로젝트 설정(Chooser 테이블 지정) 제공.

**경계 (비담당)**
- 언제 어떤 BGM 분류를 켤지 결정 — 게임플레이/BP 가 `StartBGM(태그)` 호출로 주도.
- 플레이어/적 상태(전투/보스/지역 등) 판정과 태그 부여 — 이 모듈은 ASC 의 owned-tag 를 읽기만 하며, 부여는 ASC 소유자([[WxCombat]] 등 도메인)가 한다.
- 사운드 에셋 자체의 루프/믹싱 — Sound Cue/Wave 에 위임한다.
- 데디케이티드 서버 오디오 — BGM 은 로컬 전용이라 서버에서는 아무 것도 하지 않는다.

## 의존성
- **주요 의존**: `WxCore`·`GameplayTags`·`DeveloperSettings`(Public) · `GameplayAbilities`(ASC owned-tag 이벤트/조회)·`Chooser`(테이블 평가)(Private)
...
| `FWxBGMChooserContext` | Chooser 평가 입력 struct. `PlayerStateTags` + `BGMTags` + `SourceOwnerTags` 세 Gameplay Tag 컨테이너(활성 소스가 여럿이면 합집합) | `Plugins/WxSound/Source/WxSound/Public/WxBGMChooserContext.h` |
...
## 확장 포인트 / 규약
- **새 곡 추가**: `UWxBGMData` 에셋을 만들어 `Sound`·페이드 값을 넣고, Chooser 테이블의 한 행 결과로 연결한다. 루프는 Sound 에셋(Cue/Wave)에서 켜고, 페이드 길이는 곡별로 트랙 자신이 소유한다.
- **Chooser 규약**: 테이블의 Result Class = `UWxBGMData`, Parameter = `FWxBGMChooserContext`. 세 컬럼은 모두 Gameplay Tag(`PlayerStateTags`·`BGMTags`·`SourceOwnerTags`)에 바인딩된다. 서브시스템은 승자를 미리 고르지 않으므로 곡 간 우선순위는 Row 순서로 표현한다.
...
- **국지적 BGM(보스 등)**: 액터에 `UWxBGMSourceComponent` 를 붙이고 `ActivationTag`(감시할 소유자 상태 태그)·`MusicTag`(기여 분류)를 설정. 캐릭터 코드 수정 없이 자기완결이며, 소스가 빠지면 `StartBGM` 베이스라인으로 자동 폴백된다.
...
4. `Plugins/WxSound/Source/WxSound/Public/WxBGMSourceComponent.h` — 상태 기반 국지 BGM 이 서브시스템에 끼어드는 방식.
