...
> Epic Games(Zhi Kang Shao) 제공 서드파티 예제 플러그인. World Partition 맵 상태를 여러 SaveGame 파일로 직렬화하고, 로드 시 플레이어를 마지막 맵/위치로 복귀시키는 데모다. 액터, InstancedActor, Mass, GAS, GeometryCollection, StateTree, JSON 등 각 지속성 패턴별 최소 예제를 담고 있다. ProjectWx 고유 코드가 아니라 참고용 레퍼런스다.
...
**경계 (비담당)**
- 실제 직렬화/저장 파일 관리·LSP 런타임은 위임 → `PersistenceUtils` 플러그인(엔진 예제 제공).
...
- 규칙(「WxCore 외 Wx 플러그인 참조」): 없음 ✅ — 소스 전체에 `Wx` 심볼 0건. 순수 엔진/Epic 예제 의존만 있음.
...
| `UPersistedAbilitySystemComponent` | `IPersistedObject` ASC. `Gameplay.Effect.Persistable` 태그가 붙은 활성 GE만 스냅샷/복원(로시) | `Source/PersistenceExamples/Public/GAS/PersistedAbilitySystemComponent.h` |
| `UPersistedGeometryCollectionComponent` | `IPersistedObject`. 파괴 조각별 트랜스폼·broken 상태를 캡처해 물리 프록시 재생성으로 복원 | `Source/PersistenceExamples/Public/GeometryCollection/PersistedGeometryCollectionComponent.h` |
| `ADestructibleCrate` | 위 GAS+GeometryCollection을 조합한 파괴 상자 데모 액터(ASC/HealthSet/GC 소유) | `Source/PersistenceExamples/Public/GAS/DestructibleCrate.h` |
...
| `UPersistedStateTreeComponent` | `UStateTreeAIComponent` 서브클래스. 활성 leaf state GUID 읽기/강제 전이로 StateTree 상태 지속성 지원 | `Source/PersistenceExamples/Public/AI/PersistedStateTreeComponent.h` |
...
- 복잡한 상태(파괴 포즈·GE 등)는 `IPersistedObject`를 구현하고 `PrePersistObject`/`PostRestoreObject`에서 스냅샷↔재적용(`PersistedGeometryCollectionComponent`·`PersistedAbilitySystemComponent` 참고).
- IA/Mass는 코드가 아니라 `Config/DefaultInstancedActors.ini`·엔티티 config(`UMassRepresentationTrait`의 `HighResSpawnedActor`)로 결선한다 — 헤더 doc-comment의 "To use" 지침을 따를 것.
...
---
*문서 기준 커밋 `9554c3c` · 생성일 2026-07-08 · 소스 52파일 — `/readme-writer`로 갱신*
