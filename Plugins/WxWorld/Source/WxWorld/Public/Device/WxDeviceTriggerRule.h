 ...
 *  - 탑승칸에 붙은 버튼: 지금 있는 곳을 뺀 모든 정차 지점. 플레이어가 상호작용 목록에서 고른다.
...
struct FWxDeviceTriggerRule_SplineStops : public FWxDeviceTriggerRule
{
	...
	/**
	 * 탑승칸이 이미 있는 정차 지점의 호출도 받는다. 비활성 장치를 깨우는 상태에 켠다 — 같은 층에서 깨우면 제자리에서 문만 열리고, 다른 층에서 깨우면 그 층으로 온다.
	 * 끄면 같은 층 호출은 받지 않아 그 버튼이 잠긴다(가동 중인 장치의 정지 상태).
	 */
	UPROPERTY(EditAnywhere, Category = "Wx")
	bool bAcceptCurrentStop = false;
};
