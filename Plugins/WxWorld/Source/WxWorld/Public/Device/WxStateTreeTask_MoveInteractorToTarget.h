...
struct FWxStateTreeTask_MoveInteractorToTargetInstanceData
{
	 ...
	 * 오너 장치의 InteractingCharacter 는 권위 측이 언제든 갱신하는 라이브 멤버라 진입 시점의 차단 대상을 되짚을 수 없고, 카운터는 폰이 아니라 컨트롤러에 쌓이므로 이 기록으로 짝을 맞춰야 폰이 죽어도 리스폰 후 이동이 살아난다.
	 */
	UPROPERTY()
 ...
 * 대상은 오너 장치의 InteractingCharacter 를 직접 읽는다(바인딩 입력 없음) — 값이 이미 복제되어 모든 피어가 같은 대상을 보므로 에셋에서 배선할 것이 없다.
 * 목표 = 앵커(또는 오너) 트랜스폼 ∘ 상대오프셋 이라 모든 머신에서 동일하게 계산돼, 각 피어가 자기 캐릭터 사본을 로컬 보간해도 수렴한다(별도 복제 미러 불필요).
 * 이동 중에는 로컬 플레이어의 이동 입력을 AController::SetIgnoreMoveInput 으로 막고, ExitState 가 차단을 건 대상 자체(BlockedController 기록)로 해제해 캐릭터가 소멸·언포제스돼도 스택 카운터의 짝이 맞는다.
