...
struct FWxStateTreeTask_SendEventInstanceData
{
	...
	/** 오너 BP 에 ChildActor 로 심긴 내장 장치 하나를 이름으로 지목한다. */
 ...
 *
 * 대상을 유추하지 않는다 — TargetKind 가 갈래를 정하고, 고르지 않은 쪽의 칸은 디테일 패널에서 감춘다.
 * 지목 수단이 둘인 것은 대상이 정해지는 자리가 둘이기 때문이다: 배치가 정하는 상대는 오너의 LinkedDevices 바인딩으로, 저작이 정하는 내장 장치는 이름으로 지목한다.
