 ...
 *  - 상태 식별은 엔진 순정 상태 Tag 다. 상태 디테일의 Tag 필드에 태그를 달면 그 값이 곧 식별 키가 된다(에셋 안에서 유일해야 한다).
 ...
 *  - 레이트조인·스트리밍 인도 같은 수렴이다. StateTag 만 세팅되면 트리는 루트에서 시작해 라이브 전이로 그 상태에 닿는다(그 상태로 가는 연출이 재생된다).
 *    초기 진입(SourceStateID 무효)은 BeginPlay 의 루트 진입뿐이다.
 *  - 시작 상태(InitialState)도 같은 수렴이다. 권위가 BeginPlay 에서 그 태그를 StateTag 로 삼고, 수렴할 때까지 발행을 보류해 그 값이 기본 상태 발행에 덮이지 않게 한다.
 */
UCLASS()
...
public:
	...
	/**
	 * 트리를 열기 전에 InitialState 를 권위 StateTag 로 삼아 수렴 추종에 태운다.
...
protected:
	...
	/**
	 * 지금 활성인 상태 Tag의 이름. 권위 측이 트리 틱마다 갱신한다.
	 * FGameplayTag의 중첩 TagName 대신 원형인 FName 하나를 복제 필드로 유지한다.
	 */
	UPROPERTY(VisibleInstanceOnly, ReplicatedUsing = OnRep_StateTagName, Category = "Wx")
	FName StateTagName;

	/**
	 * 시작할 상태의 Tag. 기본값 Root 는 예약어로, 에셋의 루트 기본 상태에서 시작한다는 뜻이다.
	 * 첫 진입의 StateTag 를 정할 뿐이고, 그 뒤는 같은 수렴 경로를 탄다.
	 */
...
private:
	...
	/**
	 * 트리를 틱하거나 멈춘 직후의 상태 동기화. 트리가 진실이면 발행하고(권위 평시), StateTag 가 진실이면 추종한다(클라 상시 + 권위 초기 수렴 중).
	...
	/** StateTag 를 추종한다. 어긋나 있으면 그 상태로 전이를 요청하고, 초기 수렴이 끝나면 발행으로 복귀한다. */
	void FollowStateTag();
	 ...
	 * 값은 상태 Tag(추종 키)이고, 화면에는 에디터에서 보는 상태 이름을 띄운다.
	 */
	UFUNCTION()
	...
	/**
	 * 권위 전용. 초기 상태로 미리 정해진 StateTag 가 진실인 구간 — 수렴할 때까지 발행을 보류해 그 값이 기본 상태 발행에 덮이지 않게 한다.
	 * 활성 태그가 StateTag 와 일치하는 순간 내려가고, 그때부터 다시 트리가 진실이 되어 발행으로 복귀한다.
	 */
	bool bFollowInitialState = false;
