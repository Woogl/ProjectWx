 ...
 * 상호작용 그 자체만 보므로 대상이 NPC 든 기믹이든 픽업이든 같은 노드로 판정한다 — 대화가 열리는 것은 대상의 반응이지 이 노드의 완료 조건이 아니다.
 * 한 액터에 상호작용 영역이 여럿이면(예: 엘리베이터) 어느 영역이든 성립으로 본다. 주체도 가리지 않는다(상호작용 어빌리티를 가진 것은 플레이어뿐).
 * 진입 이전의 상호작용은 세지 않는다 — 기다리는 동안 도착한 통보만 받으므로, 앞 상태를 끝낸 그 상호작용이 다음 상태까지 관통하지 못한다.
 ...
 * 대상 해석도 그 순간에 한 번만 한다. 덕분에 진입 시점에 대상이 스트리밍 아웃이어도 상관없고, 기다리는 노드가 없으면 상호작용이 일어나도 아무 일도 하지 않는다.
 *
