...
EStateTreeRunStatus FWxStateTreeTask_ComponentMove::EnterState(FStateTreeExecutionContext& Context, const FStateTreeTransitionResult& Transition) const
{
	...
	const bool bReachNow = UWxDeviceStateTreeComponent::IsRestoring(Context, Transition) || Instance.Duration <= 0.f || Component->GetRelativeLocation().Equals(Instance.TargetLocation);
	if (bReachNow)
	{
