...
| `FWxStateTreeTask_*` (기믹 노드) | 기믹 ST 공유 연출 태스크(이동·스플라인·애니·몽타주·사운드·FX·GE·입력·상호작용·스폰 등). 태스크당 파일 1개 | `Plugins/WxWorld/Source/WxWorld/Public/Gimmick/WxStateTreeTask_*.h` |
...
| `FWxStateTreeTask_TriggerSpawnersByLocator` / `_WaitSpawnersKilled` | UOL로 배치 스포너를 지정해 트리거·처치 대기하는 ST 노드(레벨 밖 호스트용) | `Plugins/WxWorld/Source/WxWorld/Public/Spawnable/WxStateTreeTask_TriggerSpawnersByLocator.h` 외 |
...
## 확장 포인트 / 규약
- **새 기믹**: 전용 액터 불필요. 임의 액터에 `UWxGimmickStateTreeComponent`를 붙이고 StateTree 에셋을 지정하면 기믹이 된다. 상태 식별은 엔진 순정 상태 Tag이며, 상태 디테일의 Tag 필드에 단 값이 곧 세이브 키다(에셋 내 유일, Tag 없는 상태는 저장 안 됨). 상태 전이는 전부 ST 에셋이 정하고 컴포넌트는 목적지를 모른다. **오너 액터의 Replicates를 반드시 켠다** — 컴포넌트는 자기 몫만 켤 수 있어 꺼져 있으면 상태 복제가 죽어 클라 기믹이 서버를 따라가지 못한다(로컬 플레이에선 정상으로 보이므로 `BeginPlay`가 Error 로그로 알린다).
- **새 연출 노드**: `FStateTreeTaskCommonBase` 파생 `FWxStateTreeTask_*`를 태스크당 헤더/소스 한 쌍(`Public|Private/Gimmick/WxStateTreeTask_<이름>.{h,cpp}`)으로 추가한다. 노드는 소유 액터의 얇은 프리미티브만 호출하고 기믹 종류를 모르며, 기믹 상태가 필요하면 컨텍스트 액터에서 `UWxGimmickStateTreeComponent`를 찾아 쓴다. 초기 진입(시작/복원/레이트조인)과 라이브 전이는 `Transition.SourceStateID` 유효성으로 구분 — 발동형 액션은 라이브에서만, 상태형 포즈는 진입 경로 무관하게 목표로 수렴, 작업 완료 시 `Succeeded` 반환.
- **재선택·완료 판정 규약**: 같은 상태가 재선택될 때(ChangeType=Sustained) 다시 돌지는 생성자의 `bShouldStateChangeOnReselect`가 선언하며, 엔진이 이 값으로 `EnterState`/`ExitState`를 대칭 게이팅한다 — 발동 순간에 한 번 일어나는 액션형(사운드·애니·스폰 트리거)은 기본값 `true`, 그 상태의 목표 포즈·가용성을 선언하는 상태형(메시 이동·상호작용/입력 토글)과 머무는 태스크는 `false`로 두어 진행 중인 작업이 재선택에 끊기지 않게 한다. 상태가 언제 완료로 판정되는지(판정에 넣을 태스크·All/Any)는 에셋이 상태별로 정하고, 완료 전이가 없는 머무는 상태는 그 태스크를 판정에서 빼야 루트 재선택 thrash 를 피한다.
...
- **리플리케이션**: 기믹 상태는 서버 권위다. 권위 측이 활성 상태 Tag를 `StateTag`(Replicated+SaveGame)에 폴링 기록하고, 클라는 틱 말미에 자기 활성 Tag와 대조해 어긋나면 그 상태로 **전이를 요청**한다(트리 재시작이 아니라 라이브 전이 — 재시작은 대기 이벤트를 날리고 진입을 초기 진입으로 만들어 트리거형 연출을 스킵시킨다). 상태를 바꾸지 않는 재진입만 값 복제로 안 보이므로 `Multicast_ReenterState`로 알린다. 상호작용 스캐너는 소유 클라 전용(복제 안 됨)이고 선택은 `ServerInteract`로 메시 포인터를 원자 전송한다.
...
2. `Plugins/WxWorld/Source/WxWorld/Public/Gimmick/WxStateTreeTask_*.h` — 기믹 ST가 조립하는 연출 노드. 태스크당 파일 하나이고 각 헤더 주석이 그 노드의 계약을 담는다(공통 규약은 위 「확장 포인트」 참조). 상태형/액션형 두 갈래를 보려면 `WxStateTreeTask_ComponentMove.h` 와 `WxStateTreeTask_PlaySound.h`.
