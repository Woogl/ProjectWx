...
- **발동 장치 연결**: 레버·버튼 같은 발동 장치는 `AWxTriggerDevice`를 상속한 BP를 배치하고 **발동 장치 쪽** `Gimmicks` 배열(레벨 인스턴스 저작)로 움직일 `AWxDevice`를 지목한다 — 하나가 여럿을(1:N), 한 장치가 여러 발동 장치에(N:1) 걸린다. 눌리면 각 장치 트리에 `Event.Interact`가 나가고, ST 에셋은 `On Event`로 받아 전이를 정한다. 발동 장치는 상시 활성이며 상태를 들지 않는다 — 상태별로 잠가야 하면 `GimmickStateRequirements`(지목 대상의 상태로 스스로 판정)나 '상호작용 켜기' 태스크의 Target 갈래를 쓴다.
