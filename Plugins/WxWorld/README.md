...
## 배치 기믹 (전부 순수 BP, `/Game/WorldObject/Gimmick/`)

기믹용 C++ 액터 클래스는 없다. 넷 다 순정 `AActor` BP 에 메시와 GimmickStateTree 컴포넌트를 붙인 것이며, 동작은 짝이 되는 ST 에셋이 전부 author 한다.
상태의 Tag 가 곧 저장 값이므로, 복원은 저장된 상태에서 트리를 여는 것으로 끝난다(각 태스크가 초기 진입을 스냅으로 처리).

| BP / ST | 상호작용 영역 | 동작 |
| --- | --- | --- |
| `BP_Door` / `ST_Door` | `Console` | Close ─상호작용→ Open. 양쪽 문이 반대로 슬라이드(Component Move, 순수 비주얼). 구조상 되닫기도 가능하나 Open 상태의 인터랙션을 에셋에서 꺼 단방향으로 쓴다 |
| `BP_Elevator` / `ST_Elevator` | `PlatformMesh`·`CallConsoleA`·`CallConsoleB` | Closed/AtStart/AtEnd 세 leaf, 각 leaf 안에서 "문 닫기 → 플랫폼 이동 → 문 열기"를 자식 상태로 choreography. 이동은 Spline Move 가 두 끝점 사이를 주파하며 self-anchoring 이라 같은 층 전이는 이동 단계가 즉시 collapse 된다. 갈 곳이 갈리는 Closed 만 전이 조건이 페이로드 Source 를 콘솔과 비교해 가른다 |
| `BP_TreasureChest` / `ST_TreasureChest` | `MeshComponent` | 한 번 열리면 끝. Open 상태가 열기 애니(Play Animation)·인터랙션 해제·보상 지급(Grant Reward)을 함께 수행한다. 지급할 보상은 이 BP 의 `RewardRow` 변수, 발사 속도·스폰 오프셋은 태스크 인스턴스 데이터 |
| `BP_CheckPoint` / `ST_CheckPoint` | `MeshComponent` | Lit 상태가 회복 GE·아이템 충전 리필·스포너 리스폰·게임 저장 넷을 수행한다. 재개 지점은 Save Game 태스크가 이 BP 의 `ResumePoint` 컴포넌트로 바인딩돼 있어, 어디서 상호작용했든 부활 자리는 그 컴포넌트다. 신규 세션 시작지점은 체크포인트가 아니라 레벨의 일반 `APlayerStart` |

## 확장 포인트 / 규약
- **신규 기믹**: 상속할 베이스 클래스가 없다 — 아무 액터(순수 BP 포함)에 `UWxGimmickStateTreeComponent`를 붙이고 ST 에셋을 할당하면 기믹이 된다. 상태 전이·연출·인터랙션 가용성은 전부 ST 에셋에서 조립한다. ST 태스크는 스키마의 Context Actor Class 를 그 BP 클래스로 지정해 컴포넌트·변수에 바인딩한다(바인딩은 이름으로 걸리므로 컴포넌트 이름을 바꾸면 끊긴다).
...
4. `/Game/WorldObject/Gimmick/ST_Elevator` — ST 에셋만으로 다상태 기믹을 어떻게 choreography 하는지 가장 풍부한 실증(leaf별 이동·문·인터랙션 조립).
