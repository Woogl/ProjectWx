...
## 책임
**담당**
- 상호작용 컴포넌트: 오버랩 감지 → 외곽선 강조 + 레지스트리 등록 → 서버 권한 `TryInteract` → Multicast 알림(`UWxInteractionComponent`).
- 로컬 플레이어별 인-레인지 상호작용 목록 레지스트리(HUD 리스트 소스 제공, `UWxInteractionRegistrySubsystem`).
...
- HUD 리스트 선택(SelectedIndex)·뷰모델 표시는 [[WxUI]]. 본 모듈은 목록만 제공한다.
...
| `UWxInteractionRegistrySubsystem` | 로컬 플레이어별 인-레인지 컴포넌트 목록. HUD 리스트 소스(`OnListChanged`) | `Source/WxWorld/Public/Interaction/WxInteractionRegistrySubsystem.h` |
...
## 여기서부터 읽어라
1. `Source/WxWorld/Public/Interaction/WxInteractionComponent.h` — 상호작용 흐름(오버랩→강조/등록→서버 TryInteract→Multicast)의 진입점, 헤더 주석에 전체 시퀀스 정리됨.
