...
## 의존성
- **주요 의존**: [[WxCore]] (`WxSavable`), GameplayAbilities, Niagara, LevelSequence/MovieScene, UMG, DeveloperSettings
- 규칙: WxCore 외 Wx 플러그인 참조 없음 ✅ (`IWxSavable`/`WxSavable.h`는 `WxCore` 소속, WxSave 플러그인 아님)
...
- **새 스폰 대상**: 스폰될 액터 클래스가 `IWxSpawnableInterface`를 구현해야 `AWxSpawner.SpawnableActorClass`에 지정 가능(`MustImplement` 제약). `OnSpawnedBy`로 빙의 전 컨텍스트 주입.
...
- **리플리케이션/권한(최대 4인 멀티)**: `TryInteract`는 서버 권한 진입점이며 `MulticastInteracted`(NetMulticast Unreliable)로 전원 fire. 프롬프트 위젯 표시는 로컬 클라이언트 전용(`IsLocalPlayerPawn`). 기믹 상태는 `ReplicatedUsing` OnRep에서 `ApplyState`를 호출하고, 서버는 `MarkTriggered`에서 명시 호출해 경로를 통일한다. WxSave 키는 에디터에서 부여된 불변 `FGuid WxSaveId`(`GetActorGuid` 기반).
...
---
*문서 기준 커밋 `2983a08e` · 생성일 2026-06-11 · 소스 24파일 — `/readme-writer`로 갱신*
