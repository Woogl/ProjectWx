...
- **새 ST 태스크**: `FStateTreeTaskCommonBase` 파생 `USTRUCT`로 만든다. 인스턴스 데이터를 짝 구조체로 두고 `using FInstanceDataType`·`GetInstanceDataType()`을 헤더에 표기(코딩 규칙 6의 유일 예외, 각 헤더 주석 참조). 태스크 분류: `Device/`(연출·이동 — 재생/컴포넌트 이동/이펙트 적용/스포너 발동·리스폰), `Interaction/`(상호작용 켜기·대기), `Spawnable/`(로케이터 지정 스포너 발동·처치 대기).
...
- **발동 장치 연결**: 레버·버튼 같은 발동 장치는 `AWxTriggerDevice`를 상속한 BP를 배치하고 **발동 장치 쪽** `LinkedDevices` 배열(레벨 인스턴스 저작)로 움직일 `AWxDevice`를 지목한다 — 하나가 여럿을(1:N), 한 장치가 여러 발동 장치에(N:1) 걸린다. 장치 BP의 ChildActorComponent로 심긴 발동 장치는 그 장치를 스스로 지목하므로(BP_Door의 내장 레버) 배선이 필요 없다. 눌리면 각 장치 트리에 `Event.Interact`가 나가고, ST 에셋은 `On Event`로 받아 전이를 정한다. 발동 장치는 상시 활성이며 상태를 들지 않는다 — 상태별로 잠가야 하면 `StateTagRequirements`(지목 대상의 상태로 스스로 판정)나 '상호작용 켜기' 태스크의 Target 갈래를 쓴다.
- **발동 연출**: `AWxTriggerDevice`는 생김새·연출을 알지 않는다. 눌리면 전 피어에서 `OnTriggered`(BlueprintImplementableEvent)가 불리므로 파생 BP가 Timeline·사운드로 손잡이 회전 같은 것을 재생한다. 재조작 차단은 연출 길이가 아니라 `RetriggerCooldown`이 들므로 둘을 맞춰 둔다.
