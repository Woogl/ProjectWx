...
- StateTree 기반 퀘스트 로직의 향후 구현 컨테이너 및 `LogWxQuest` 로그 카테고리 정의
...
| 타입 | 역할 | 위치 |
| --- | --- | --- |
| `FWxQuestModule` | 모듈 진입점 (현재 Startup/Shutdown 빈 구현) | `Source/WxQuest/Public/WxQuestModule.h` |
| `LogWxQuest` | 모듈 전용 로그 카테고리 | `Source/WxQuest/Public/WxQuestModule.h` |

> 퀘스트 도메인 타입(퀘스트/목표 클래스, DataTable Row, StateTree 스키마 등)은 아직 없음. 신규 코드 추가 시 이 표를 갱신할 것.

## 확장 포인트 / 규약
- 현재 퀘스트 도메인 타입은 미정의 상태다. 신규 구현은 `Public/`에 헤더를, `Private/`에 구현을 두는 표준 모듈 구조와 `Wx` prefix 규칙을 따른다.
...
## 여기서부터 읽어라
1. `WxQuest.uplugin` — 모듈 의도(StateTree 기반 퀘스트 시스템)와 플러그인 의존성(StateTree, WxCore) 확인
2. `Source/WxQuest/WxQuest.Build.cs` — 빌드 의존 모듈(StateTreeModule, GameplayTags, DeveloperSettings) 확인
3. `Source/WxQuest/Public/WxQuestModule.h` — 모듈 진입점, 이후 타입 추가의 시작점
...
---
*문서 기준 커밋 `9554c3c` · 생성일 2026-07-08 · 소스 2파일 — `/readme-writer`로 갱신*
