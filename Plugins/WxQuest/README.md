...
- 퀘스트 전용 ST 에셋 타입으로 일반 ST 오지정을 픽커·컴파일 단계에서 차단 (`UWxQuestStateTree`)
...
## 의존성
- **주요 의존**: `WxCore`(`Quest.Fail` 등 이벤트 태그), StateTree / GameplayStateTree(러너·노드 베이스), ModularGameplay(`UGameStateComponent`), UniversalObjectLocator(레벨 배치 액터 지정)
...
## 확장 포인트 / 규약
- **새 퀘스트**: `UWxQuestStateTree` 에셋 생성(WxEditor 팩토리). 탑재는 레벨 배치 시작 볼륨 등이 `UWxQuestLibrary::ActivateQuest` 로 지정하고, 그 뒤 진행 개시는 퀘스트 자신의 Start 게이트 태스크가 판정한다. 컴포넌트는 에셋 불가지 — 실행 대상은 전부 데이터가 지정한다.
...
- **저널 태스크의 완료 판정 참여는 얹히는 상태로 갈린다** — SetQuestTitle 은 자식을 둔 상태에 홀로 얹혀 판정에 참여하고(빠지면 형제 완료를 물려받아 진입 즉시 완료로 오독), SetQuestObjective 는 Wait 태스크와 같은 상태에 얹혀 `bConsideredForCompletion=false`(끼면 그 상태가 영영 완료 안 됨). 둘 다 완료를 내지 않고 상주하며, 상태 완료는 짝이 되는 Wait 태스크가 낸다. 어떤 상태든 완료 판정 태스크를 최소 하나는 가져야 한다.
...
- **리플리케이션**: 러너·저널은 서버 권위(v1 싱글/리슨 호스트, 0번 컨트롤러 폰 전제). 비-권위 머신에선 러너가 null 이고 라이브러리 호출은 내부에서 무시된다.
...
## 관련
- 상위: [[WxCore]] (`Quest.Fail` 등 이벤트 태그), Experience 에셋(컴포넌트 주입), 레벨 트리거 볼륨·[[WxDialogue]](라이브러리 호출로 개시/이벤트 전송)
- 소비: [[WxUI]] (저널 델리게이트 구독)

---
*문서 기준 커밋 `bb8ee6b` · 생성일 2026-08-07 · 소스 9파일 — `/readme-writer`로 갱신*
