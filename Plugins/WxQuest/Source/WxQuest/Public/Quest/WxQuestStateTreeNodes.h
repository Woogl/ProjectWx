 ...
 * 여럿을 지정하면 그중 하나에만 닿아도 된다 — 목적지 후보가 여럿인 도착 목표를 그대로 옮긴 것이며, 같은 상태에 얹히는 MarkIndicators 도 그 후보들을 함께 가리킨다.
 * 대상 미해석(스트리밍 아웃)·폰 부재 동안은 그 대상만 판정에서 빠진다. 0번 컨트롤러 사용은 GiveRewards 등 기존 크로스모듈 노드와 같은 전제(v1 싱글/리슨 호스트)다.
