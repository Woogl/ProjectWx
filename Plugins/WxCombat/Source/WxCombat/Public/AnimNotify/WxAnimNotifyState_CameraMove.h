 ...
 *
 * NotifyBegin~NotifyEnd 구간 동안 로컬 플레이어의 뷰를, 몽타주를 재생하는 액터(플레이어 또는 적)의
 * 메시 기준 상대 위치에 스폰한 임시 카메라로 블렌드 전환하고, 구간이 끝나면 로컬 플레이어 폰으로
 * 블렌드 복귀한다. 플레이어 피니셔, 적 패턴 등 그 순간을 멋있는 고정 각도로 보여주는 용도.
 *
 * 순수 로컬 연출이라 이 클라이언트의 로컬 플레이어 뷰에만 적용된다. 특히 플레이어가 재생한 몽타주는
 * 그 플레이어 본인 화면에만 적용돼(리플리케이트된 남의 몽타주는 무시), 멀티플레이에서 남의 피니셔가
 * 내 카메라를 흔들지 않는다. 적/AI 몽타주는 각 클라의 로컬 플레이어 뷰에 적용된다.
...
protected:
	...
	/** 프리뷰 카메라 배치 기준 트랜스폼. 부착 모드는 실시간 몸체(매 틱 갱신), 고정 모드는 구간 첫 진입 스냅샷(유지). 정지 중 편집 즉시 반영(PostEditChangeProperty)에도 쓴다. */
