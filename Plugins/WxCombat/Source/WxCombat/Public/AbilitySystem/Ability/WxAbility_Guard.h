...
/**
 * 가드 홀드 상태를 소유한다 — Effect.Guard 부여와 루핑 자세 몽타주가 전부다.
 *
 ...
 *
 * 가드 반격은 아직 성립하지 않는다. 이 어빌리티가 배타 점유를 놓지 않고(가드 몽타주에 StartRecovery가 없다) 공격 쪽도 Ability.Guard를 지목하지 않아 가드 중에는 공격이 발동하지 못하며, Effect.Guard로 반격 세트를 고르는 공격 어빌리티도 아직 없다.
 */
UCLASS(Abstract)
