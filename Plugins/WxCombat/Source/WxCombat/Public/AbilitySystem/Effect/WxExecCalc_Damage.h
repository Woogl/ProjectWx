 ...
 *
 * 판정 흐름:
 *  1. 고정 대미지 모드 → SetByCaller.FixedDamage 가 양수이면 전투 파이프라인을 우회하고
 *     IncomingDamage·AI 감지·GameplayCue 만 처리한다 (낙사·장판 등 환경 대미지 용도).
 *     무적이면 PerfectDodge 보상 없이 조용히 차단.
 *  2. 무적 판정 → 대미지 무효, 회피 성공 보상
 *  3. 어트리뷰트 캡처 (ATK, DEF, CritRate, CritDMG)
 *  4. 상태 판정 (퍼펙트 가드, 가드, Unblockable)
 *  5. 대미지 적용
...
private:
	/** 고정 대미지 처리. ATK/DEF/Crit/가드/HitReact 등 전투 파이프라인을 우회하고 IncomingDamage·AI·Cue만 발동 */
	void ExecuteFixedDamage(UAbilitySystemComponent* SourceASC, UAbilitySystemComponent* TargetASC, const FGameplayEffectSpec& OwningSpec, float FixedDamage, FGameplayEffectCustomExecutionOutput& OutExecutionOutput) const;
