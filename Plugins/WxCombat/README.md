...
- 전투 어빌리티: 공격·회피·가드·스킬·궁극기·피니셔·히트리액트·그로기·사망·락온·스프린트·AI 패턴 등 (`AbilitySystem/Ability/WxAbility_*`)
- 대미지 파이프라인: `FWxDamageInfo` 설계 데이터 → GE Spec 변환 → `WxExecCalc_Damage` 산출 → `PostGameplayEffectExecute` HP 차감
- GameplayEffect/계산 모듈군: `WxEffect_*`(대미지·코스트·쿨다운·번·리소스·치트), `WxExecCalc_*`, `WxMMC_*`, GameplayCue(`AbilitySystem/Cue/`)
- 무기·투사체: `AWxWeaponBase`, `AWxProjectileBase`
- 히트박스/전투 타이밍 AnimNotify: `WxAnimNotify*`(무기 공격창·콤보 윈도·무적·퍼펙트가드·투사체 스폰·카메라 무빙 등)
...
**경계 (비담당)**
- 공용 정의·팀/태그 등 파운데이션은 [[WxCore]]에 위임 (이 모듈은 GameplayTag를 소비만 하고 C++ Native 선언은 두지 않는다)
...
- 규칙: WxCore 외 Wx 플러그인 참조 — 없음 ✅ (Build.cs·uplugin 의존은 `WxCore` 하나뿐)
...
| `UWxCombatLibrary` | 무기/투사체 밖 경로의 단일 대미지 적용 진입점(`ApplyDamage`) | `Source/WxCombat/Public/WxCombatLibrary.h` |
...
## 확장 포인트 / 규약
- **새 어빌리티**: `UWxAbilityBase` 상속. `EWxAbilityActivationPolicy`로 트리거형/부여즉시형 선택. 입력 발동이면 `ActivationInputAction` 지정(복수 입력은 `IsActivationInput` override). 쿨다운·코스트 수치는 코드가 아닌 `FWxAbilityTableRow`(`AbilitySystem/Ability/WxAbilityTableRow.h`)에서만 읽는다. 쿨다운은 공용 `UWxEffect_Cooldown` GE + 소스 어빌리티 CDO로 구분(마커를 다른 GE로 바꾸면 엔진 순정 경로).
...
- **리플리케이션**: 어트리뷰트·락온 대상·최근 입력·타임딜레이션·투사체 스폰/파괴는 서버 권위. 락온은 소유 클라 예측 후 복제값으로 정합.
...
4. `Source/WxCombat/Public/WxDamageInfo.h` + `Source/WxCombat/Private/AbilitySystem/Effect/WxExecCalc_Damage.cpp` — 대미지가 설계 데이터에서 최종 수치로 흐르는 경로

## 관련
- 상위: 전투 어빌리티를 구동하는 [[WxAI]], 전투 상태를 표시하는 [[WxUI]] · Experience/캐릭터 BP가 `UWxAbilitySet`을 지정해 이 시스템을 부팅 · 공용 정의는 [[WxCore]]

---
*문서 기준 커밋 `c9c7765` · 생성일 2026-08-04 · 소스 145파일 — `/readme-writer`로 갱신*
