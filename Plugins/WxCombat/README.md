...
- 입력 라우팅과 버퍼링, 히트스톱, 몽타주 재생 속도(ASPD) 관리 (`UWxAbilitySystemComponent`)
...
| 타입 | 역할 | 위치 |
| --- | --- | --- |
| `UWxAbilitySystemComponent` | ASC. 입력 라우팅·버퍼·히트스톱·몽타주 속도의 단일 진입점 | `Source/WxCombat/Public/AbilitySystem/WxAbilitySystemComponent.h` |
...
## 여기서부터 읽어라
1. `Source/WxCombat/Public/AbilitySystem/WxAbilitySystemComponent.h` — 입력·버퍼·히트스톱이 만나는 전투의 심장. 어빌리티 라우팅 진입점
