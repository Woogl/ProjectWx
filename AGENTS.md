...
* WX 프로젝트에서 사용자가 제작을 요청한 3D 모델링 결과물은 `C:\Users\user\OneDrive\바탕 화면\FBX모델` 경로에 직접 저장한다. 프로젝트 폴더 안에는 저장하지 않는다.
* 별도의 `OUTPUT` 폴더를 만들지 않는다.
...
2. **계획 기록**: 승인 직후·코드 착수 전, `.codex/worklog/YYYY-MM-DD-제목.md`를 새로 작성해 아래 형식으로 「계획」을 채운다. 승인받은 plan이 곧 「계획」 내용이다. (plan mode에선 파일 쓰기가 막히므로 승인 후 기록한다.) 별도 템플릿 파일은 사용하지 않는다.
