...
* 총 7부/26개의 단락으로 매우 거대하게 구성되어 있습니다. 지나치게 내용이 길어서 핵심 규칙을 찾기 힘듭니다.

* 서두에서는 PC와 적의 경직 시스템이 규칙이 공통이라고 설명하고서는, 정작 본문에서는 PC 항목과 적 항목을 나누어 설명하여 헷갈림을 유발합니다.

* 단락 인덴팅 규칙이 일관되지 않습니다.
  * 예: "경직 면역 상태"를 설명할 때 PC 항목에서는 소단락인 6.3을 사용했지만, 적 항목에서는 대단락인 15를 사용.

## 문제점 2. 내용 중복

* 앞에서 이미 설명한 내용을 잠시 후에 다시 재정의하거나, 조금씩 다르게 설명하여 독자에게 피로감과 혼동을 줍니다.
  * 예: "일반 상태"가 1, 2, 6, 9, 12, 13, 25, 26 번 단락에서 반복해서 재정의됨 (총 8회)
  * 예: `PC는 패턴 상태를 사용하지 않는다.` 라는 문장이 3, 7, 8, 25번 단락에서 똑같이 반복됨 (총 4회)
  * 그 외 다수
...
* "상태" 관련
  ...
  * 가장 치명적인 문제점으로, 문서의 설명 기준이 "상태"라는 것에 얽매여있어 구조가 이상해졌습니다.

* "경직 우선순위" 관련
  * 경직 판정이 여러 개 동시에 발생할 수 없습니다.
  * 대미지 프로세스는 동기적(Synchronous)으로 실행되기 때문에 절대 다중 실행되지 않습니다.
...
* "경직 종류" 관련
  * 표에서 분류 축이 2개가 뒤섞여 있습니다. (경직 원인에 따른 분류 / 모션에 따른 분류)
  * 종류에 따른 경직을 구현할 때, 어떤 경직을 써야하는지 알 수가 없습니다.

## 문제점 4. 다이어그램 오남용

* 1개의 통합 다이어그램으로 설명 가능한 부분을, 5개의 다이어그램으로 쪼개어놓았습니다.

* 다이어그램이 하나의 부에 모여 있는 것도 아닐 뿐더러, 이곳 저곳에 파편화되어 있어서 한눈에 파악할 수 없습니다.
...
애초에 우리 게임은 PC와 적의 데미지 파이프라인 및 경직 규칙이 똑같아서, 여러개를 그릴 필요가 없습니다.
...
flowchart TD
  ...
  E1'([퍼펙트 가드 발생]):::entry --> Q1'{공격자가 슈퍼아머<br/>적용 중인가?}:::branch

  Q1' -- Yes --> R1'[공격자에게 DP 대미지만 적용]:::result
  Q1' -- No --> Q2'{타격에 패링 리액션<br/>있는가?}:::branch

  Q2' -- Yes --> R3'[공격자에게 DP 대미지 적용<br/>패링 리액션 발동<br/>어빌리티 캔슬]:::result
  Q2' -- No --> R2'[공격자에게 DP 대미지만 적용]:::result
```
