...
> 11파일짜리 작은 모듈로 책임 분리(호스트 액터 / 대화 정의 / PC 세션)가 선명하고, 헤더 주석이 결정의 근거까지 남겨 두어 전반적으로 건강하다. `CLAUDE.md` 위반은 0건이다 — 저작권 첫 줄 11파일 전부 통과, `Wx` prefix 일관, `BlueprintCallable`·`FORCEINLINE`·헤더 인라인 정의 0건(유일한 헤더 본문 `GetInstanceDataType()` 은 규칙 6 의 명시 예외이고 사유 주석이 붙어 있다), 유일한 람다도 예외 사유를 달았으며, 델리게이트 콜백은 `HandlePoseLoaded` 하나로 `Handle` prefix 를 지킨다. 플러그인 의존은 `WxCore` 뿐이다. 남은 문제는 전부 세션의 수명·종료 경로에 몰려 있다. 이번 리뷰는 모듈 11파일을 모두 읽고 세션 컴포넌트 cpp 와 StateTree 태스크를 정독했으며, 소비자(`WxViewModel_Dialogue`·`WxUIManagerSubsystem`·`WxAbility_Interact`·`WxRespawnLibrary`·`AWxNpc`)와 엔진의 `TStateTreeStrongExecutionContext::FinishTask`·`AActor` 기본 틱 플래그는 계약 검증 목적으로만 교차 확인했다.
...
- **범주**: 버그/정확성
- **문제**: 컴포넌트에 `EndPlay`·`UninitializeComponent` 오버라이드도, 컨트롤러의 폰 교체를 듣는 지점도 없다(`Public/WxDialogueSessionComponent.h:113-173` 의 private 섹션 전체에 그런 멤버가 없다). 그래서 세션을 끝낼 수 있는 주체는 뷰가 부르는 `Advance()`(`Source/WxGame/MVVM/WxViewModel_Dialogue.cpp:48`)와 "다음 대화의 시작" 둘뿐이다. 그런데 대화 도중 사망·리스폰이 나면 `UWxRespawnLibrary::RequestRespawn` 이 폰을 언포제스하고(`Source/WxGame/Framework/WxRespawnLibrary.cpp:47`) 파괴하며(`:65`), 그 폰 교체를 받은 `UWxUIManagerSubsystem::HandlePossessedPawnChanged`(`Plugins/WxUI/Source/WxUI/Private/System/WxUIManagerSubsystem.cpp:235`)가 `WatchPawnTags` 초입에서 대화 창을 닫아 버려(`:252`) `Advance()` 를 부를 주체 자체가 사라진다. 대화 중 무적·정지 처리는 어디에도 없어(`State.Dialogue` 를 보는 곳은 상호작용 차단과 UI 뿐) 실제로 일어날 수 있는 상황이다. 세션은 `CurrentRowName` 을 든 채 영구히 "진행 중"으로 남고, 파급이 셋이다.
...
- **범주**: 설계/구조
- **문제**: `EndDialogue()` 의 `OnDialogueEnded.Broadcast()` 는 정상 완주와 비정상 종료를 구분하지 않고, 신호에 사유가 실리지 않으므로 `Play Dialogue` 태스크의 리스너는 무조건 `FinishTask(EStateTreeFinishTaskType::Succeeded)` 를 낸다. 그런데 `EndDialogue()` 로 가는 길 중 셋이 완주가 아니다.
  ...
  - `:80` — `NextRow` 가 가리키는 행을 찾지 못했거나 그 행의 대사가 비어 있는 경우다. 바로 위에서 "정상 종료가 아니다"라고 경고까지 찍고(`:77-79`) 성공으로 보고한다. 즉 대화 테이블의 `NextRow` 오타 하나가 퀘스트를 그대로 통과시킨다.
...
- **확신도**: 높음 (경로는 코드로 확정. `Succeeded` 처리가 의도된 관용일 수는 있으나 `:77-79` 의 경고 문구와 `Public/WxStateTreeTask_PlayDialogue.h:29` 의 규정과 어긋난다)
...
- **범주**: 버그/정확성
- **문제**: `Broadcast()` 직후 `Clear()` 라, 종료 통보를 받은 리스너가 그 자리에서 새 대화를 열고 새 약속을 붙이면 그 약속까지 함께 지워진다 — 그 대화는 종료 통보를 영영 못 받는다. 지금은 유일한 구독자인 `Play Dialogue` 태스크가 완료를 뒤로 미루므로 이 재진입이 실현되지 않는다(엔진 `TStateTreeStrongExecutionContext::FinishTask` 는 `bHasPendingCompletedState` 만 세우고 전이는 예약된 다음 Tick 에 돈다 — UE 5.8 `StateTreeAsyncExecutionContext.cpp:245-249` 확인).
...
- **범주**: 중복/복잡도
- **문제**: `UniversalObjectLocator` 를 `PublicDependencyModuleNames` 에 넣었지만 모듈 소스 어디에서도 UOL 타입을 쓰지 않는다(모듈 전 소스 검색 히트 0건 — Build.cs 자신의 한 줄이 유일하다). 나머지 의존은 모두 실사용된다(`GameplayAbilities`·`GameplayTags` = 폰 ASC 루즈 태그, `StateTreeModule` = 태스크).
- **제안**: 제거한다.
- **확신도**: 높음
