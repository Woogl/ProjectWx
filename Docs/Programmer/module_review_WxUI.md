...
> 수명·구독 해제 규약이 주석으로 명문화되고 모듈 경계도 깨끗한(WxCore 외 Wx 참조 0건) 건강한 모듈이다. 직전 리뷰의 지적 중 「HUD 중복 push」와 「MarkIndicators 매 틱 UOL 재해석」 두 건은 코드에서 해소됐고, 나머지는 현재 코드로 재확인해 그대로 유효하다. 이번 리뷰는 `UWxUIManagerSubsystem`·뷰모델 계층 전체·인디케이터 매니저/ST 노드·자막 ST 노드·팝업·탭 리스트를 cpp까지 깊게 보고, 위젯 베이스·설정·모듈 파일은 훑었다(BP/WBP 내부는 범위 밖).
...
### 1. 🟡 어트리뷰트·어빌리티 VM 캐시 키가 요청을 온전히 식별하지 못한다
...
### 4. 🟡 규칙 위반 — 위젯·뷰모델 클래스의 `BlueprintCallable` 사용 범위
...
### 8. 🟢 팝업·비동기 push의 실패 경로가 아무 신호를 내지 않는다
...
### 11. 🟢 `Targets`가 런타임에 줄면 등록증이 해제 없이 버려진다
...
- **확인 후 위반 0건**: 모듈 경계(`.uplugin`·`Build.cs`·전 include 목록 모두 Wx 중 `WxCore`만 참조 — `Plugins/WxUI/Source/WxUI/WxUI.Build.cs:27`, 다른 도메인 플러그인은 주석 언급 1건뿐) · 전 소스 64개 첫 줄 Copyright 주석 · `Wx` prefix · `FORCEINLINE` 0건(헤더 정의는 StateTree `GetInstanceDataType` 2건과 템플릿 `PushWidgetToLayerStack` 1건뿐이며 앞의 둘은 파일 주석에 예외 근거 명시) · 델리게이트 콜백의 `Handle` prefix · override의 `Super::` 호출 누락 없음 · 람다 3건(`Plugins/WxUI/Source/WxUI/Private/WxUILibrary.cpp:24` 다이나믹→네이티브 델리게이트 브릿지, `Plugins/WxUI/Source/WxUI/Private/Widget/WxTabListWidgetBase.cpp:43` `FindByPredicate`, `Plugins/WxUI/Source/WxUI/Private/System/WxUIManagerSubsystem.cpp:142` CommonUI `AddWidget` 초기화 콜백)은 모두 API상 필요한 자리
- **직전 리뷰 대비 해소**: (a) 폰 교체 시 HUD 중복 push — `GameHUD` 약참조를 기억해 재push를 막고(`WxUIManagerSubsystem.cpp:277-282`), layout 재생성 시 함께 리셋하도록(`:250`) 고쳐졌다. (b) `Mark Indicators`의 매 틱 UOL 재해석 — 등록증·대상 컴포넌트 유효성 검사가 `ResolveTargetActor` **앞으로** 옮겨져(`WxIndicatorStateTreeNodes.cpp:55-64`) 정상 표시 중에는 해석이 돌지 않는다.
- **미검토 / 한계**:
  - `UWxNameplateComponent::InitializeViewModels`(`Plugins/WxUI/Source/WxUI/Private/Component/WxNameplateComponent.cpp:106`)의 `InASC` 널 가드 부재는 `.claude/worklog/2026-07-24-네임플레이트-표시판정-이벤트-구동화.md`에서 "유일 호출자가 기본 서브오브젝트를 넘기므로 호출자 없는 방어 가드는 지양"으로 기각됐다. 호출자가 `Source/WxGame/Character/WxEnemyCharacter.cpp:42` 하나뿐임을 재확인했고 발견으로 올리지 않았다. `EndPlay`가 Character VM을 `Deinitialize()` 하지 않는 것도 ASC가 함께 파괴되며 티커·구독이 다음 틱에 자멸하므로 제외했다.
