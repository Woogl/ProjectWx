...
> 수명·구독 해제 규약이 주석으로 명문화된 건강한 모듈이다. 직전 리뷰(2026-08-05) 이후 코드 변경은 인디케이터 ST 태스크의 다중 대상화(`MarkIndicators`) 한 건뿐이고, 그 외 지적은 손대지 않아 대부분 그대로 유효하다. 이번 리뷰는 `UWxUIManagerSubsystem`·VM 계층 전체·인디케이터 매니저/신규 `MarkIndicators` 노드·자막 ST 노드를 cpp까지 깊게 보고, 위젯 베이스·설정·모듈 파일은 훑었다(BP/WBP 내부는 범위 밖).
...
- **범주**: 성능/안전
- **문제**: `Tick` → `RefreshIndicators` → `ResolveTargetActor` 순으로 매 프레임 `FUniversalObjectLocator::SyncFind`가 돈다. "이미 등록돼 있는가" 검사(`:59`)는 해석이 끝난 **뒤에** 하므로, 정상 표시 중에도 경로 해석 비용을 계속 낸다. 대상이 배열이 되면서(`FWxStateTreeTask_MarkIndicatorsInstanceData::Targets`) 이 비용이 대상 수만큼 곱해진다. 월드 파티션 스트리밍을 그대로 따라가려는 의도는 주석에 명시돼 있지만, 등록 이후에는 등록증이 대상 컴포넌트를 이미 들고 있어(`Plugins/WxUI/Source/WxUI/Public/Indicator/WxIndicatorDescriptor.h`의 `TargetComponent`) 해석 결과가 바뀌는 경우가 드물다.
...
### 11. 🟢 인디케이터 위젯이 배열 인덱스로 묶여 있어 등록 순서가 곧 표시 대상이 된다
...
- **범주**: 설계/구조
- **문제**: `UWxViewModel_Indicator`는 `Registered[IndicatorIndex]`로 자기 등록증을 찾는데, 매니저의 `Indicators`는 등록 순서 배열이고 `Remove()`는 순서를 유지한 채 뒤 원소를 당긴다. `MarkIndicators`가 대상 배열을 지원하게 되면서(`FWxStateTreeTask_MarkIndicatorsInstanceData::Targets`) 다중 표시가 기본 시나리오가 됐는데, 이 배열의 인덱스는 `Targets` 순서가 아니라 "먼저 해석에 성공한 순서"다 — 앞 대상이 스트리밍 아웃돼 있으면 뒤 대상이 0번을 차지하고, 나중에 앞 대상이 로드되면 1번에 붙는다. 해제 시에도 뒤 원소가 당겨져 각 위젯이 담당하던 대상이 서로 바뀐다. 위젯 외형이 같으면 눈에 띄지 않지만, 위젯별 애니메이션·전환 상태는 그 순간 리셋된다.
...
- **확인 후 위반 0건**: 모듈 경계(`.uplugin`·`Build.cs`·include 모두 Wx 중 `WxCore`만 참조 — `Plugins/WxUI/Source/WxUI/WxUI.Build.cs:27`) · 전 소스 64개 첫 줄 Copyright 주석 · `Wx` prefix · `FORCEINLINE` 0건 · 헤더 인라인 정의는 StateTree `GetInstanceDataType` 2건뿐이며 파일 주석에 예외 근거 명시(`PushWidgetToLayerStack`은 템플릿이라 헤더 정의가 불가피) · 델리게이트 콜백의 `Handle` prefix · 람다 3건(`Plugins/WxUI/Source/WxUI/Private/WxUILibrary.cpp:25` 다이나믹→네이티브 델리게이트 브릿지, `Plugins/WxUI/Source/WxUI/Private/Widget/WxTabListWidgetBase.cpp:44` `FindByPredicate`, `Plugins/WxUI/Source/WxUI/Private/System/WxUIManagerSubsystem.cpp:145` CommonUI `AddWidget` 초기화 콜백)은 모두 API상 필요한 자리 · override의 `Super::` 호출 누락 없음
- **직전 리뷰(2026-08-05) 대비 변경**: 코드 변경은 `MarkIndicator`→`MarkIndicators` 다중 대상화 한 건이며, 그로 인해 항목 5(대상 수만큼 곱해지는 해석 비용)·항목 11(다중 표시가 기본 시나리오가 됨)의 근거를 갱신하고 항목 12를 새로 올렸다. 나머지 지적은 코드가 그대로라 유효성을 재확인해 유지했다.
- **미검토 / 한계**:
  - `UWxNameplateComponent::InitializeViewModels`(`Plugins/WxUI/Source/WxUI/Private/Component/WxNameplateComponent.cpp:88`)의 `InASC` 널 가드 부재는 `.claude/worklog/2026-07-24-네임플레이트-표시판정-이벤트-구동화.md`에서 "유일 호출자(`Source/WxGame/Character/WxEnemyCharacter.cpp:44`)가 기본 서브오브젝트를 넘기므로 호출자 없는 방어 가드는 지양"으로 기각됐다. 호출자가 하나뿐임을 재확인했고 발견으로 올리지 않았다.
  - `PrimaryGameLayout`·`TrackedPlayerController`가 단일 필드라 로컬 플레이어가 둘 이상이면 나중 플레이어가 앞선 플레이어의 레이아웃을 철거한다(`Plugins/WxUI/Source/WxUI/Private/System/WxUIManagerSubsystem.cpp:248`). 코드·README·ST 노드 주석이 모두 "v1 싱글/리슨 호스트 전제"를 명시하므로 의도된 제약으로 보아 발견으로 올리지 않았다.
...
---
*문서 기준 커밋 `95a57ef3` · 리뷰일 2026-08-07 · 소스 64파일 — `/module-review`로 갱신*
