...
> 여전히 건강한 모듈이다. 비동기 push 재진입 가드, 이미지 요청 슬롯 무효화, 스크린 투영 접힘 보정, 공유 뷰모델의 Outer 규약 같은 까다로운 지점이 의도적으로 방어돼 있고 그 근거가 대부분 주석에 남아 있다. 이번 리뷰는 `Build.cs`·`uplugin` 의존 경계부터 UI 매니저·비동기 push·플레이어 레이아웃 컴포넌트·뷰모델 9종·인디케이터 투영·네임플레이트·팝업/버튼 위젯·StateTree 노드 2종까지 cpp 본문을 포함해 봤고, 직전 리뷰(`6ea7624`)의 발견은 하나도 손대지 않아 그대로 남아 있다. 새로 더한 것은 미커밋 작업본이 만든 뷰모델 파괴 경로 1건이고, 직전 발견 중 `TryActivateAbility` 의 `BlueprintCallable` 은 프로젝트가 승인한 VM Command 예외에 해당해 걷어냈다.
...
- **위치**: `Plugins/WxUI/Source/WxUI/Private/System/WxUIManagerSubsystem.cpp:66-79`, `Plugins/WxUI/Source/WxUI/Private/System/WxUIManagerSubsystem.cpp:299-305`, `Plugins/WxUI/Source/WxUI/Private/WxUILibrary.cpp:84-102`
- **범주**: 버그/정확성
- **문제**: `ShowConfirmation` 은 `SetBeforePushCallback` 만 걸고 `SetCompletionCallback` 은 걸지 않는다. 그래서 push 가 실패하는 모든 경로 — 클래스 미지정(`WxAsyncAction_PushWidgetToLayer.cpp:24-28`), 레이아웃 부재(`:33-37`), 로드 중 레이아웃 교체(`:100-104`), 위젯 생성 실패(`:113-118`), 레이어 태그 오지정(`:127-131`) — 에서 `ResultCallback` 이 한 번도 실행되지 않고 로그도 남지 않는다. 클래스 오지정도 같다: 로드된 위젯이 `UWxGamePopup` 이 아니면 `HandleConfirmationPopupReady` 의 `Cast`(`:301`)가 실패해 `SetupPopup` 이 건너뛰어지고, 버튼이 연결되지 않은 팝업이 뜬 채 콜백도 오지 않는다. `UWxUILibrary::ShowConfirmationPopup` 은 BP 노출 API 라, 결과를 기다려 흐름을 재개하는 BP 는 그대로 멈춘다.
...
### 2. 🟡 `UWxViewModel_Ability::Deinitialize` 가 파괴 경로에서도 표시 필드를 브로드캐스트한다
- **위치**: `Plugins/WxUI/Source/WxUI/Private/MVVM/WxViewModel_Ability.cpp:104-117`
- **범주**: 설계/구조 (베이스 계약 위반)
- **문제**: 베이스가 `Deinitialize` 의 계약을 "파생은 자기 정리 후 Super 를 호출한다", "표시 필드 변경은 브로드캐스트하지 않는다 — `BeginDestroy` 에서도 불려, 함께 수거될 뷰의 바인딩으로 통지가 들어간다"로 못박아 뒀는데(`WxViewModel.h:40-43`, 호출부 `WxViewModel.cpp:31-35`), 어빌리티 VM 은 `Super::Deinitialize()`(104행) 뒤에 `Set*` 12줄을 붙여 매 필드마다 `UE_MVVM_SET_PROPERTY_VALUE` 로 통지를 쏜다. 같은 상황을 형제 VM 두 곳은 명시적으로 막아 뒀다 — `UWxViewModel_Character::Deinitialize` 는 `if (!HasAnyFlags(RF_BeginDestroyed))` 로 감싸고(`WxViewModel_Character.cpp:51-57`), `UWxViewModel_Item::Deinitialize` 는 같은 플래그로 통지 없는 분기를 탄다(`WxViewModel_Item.cpp:31-37`). 이 12줄은 미커밋 작업본에서 들어온 변경이라 커밋 `53bc7de6` 에는 없다. 부수적으로 `Initialize` 첫 줄이 `Deinitialize()` 라(`:17`), 슬롯 VM 을 다시 초기화하면 "빈 값" 한 벌이 먼저 방송된 뒤 다시 채워진다.
- **제안**: 형제 VM 과 같이 `RF_BeginDestroyed` 로 감싸거나, 통지가 필요 없다면 `UE_MVVM_SET_PROPERTY_VALUE` 대신 필드에 직접 대입한다. 호출 순서도 계약대로 자기 정리를 먼저 하고 `Super::Deinitialize()` 를 마지막에 부르는 편이 맞다.
- **확신도**: 높음 (계약과 형제 구현에서 어긋난다는 사실은 확실하다. 실제 크래시로 이어지는지는 확인하지 않았다)
...
### 4. 🟡 PlayerController 가 교체되면 걸어 둔 게임 정지를 풀 경로가 없다
...
- **범주**: 설계/구조 (상태 관리)
- **문제**: `HandlePlayerControllerSet` 은 `TrackedPlayerController.Reset()`(210행)을 **레이아웃 철거(214-218행)보다 먼저** 한다. 그래서 철거로 위젯이 비활성화되며 들어오는 통지에도 `RefreshGamePause` 는 `if (!PC) return;`(141-145행)에서 빠져나가고, 나가는 PC 에 `SetPause(false)` 를 부르는 코드는 어디에도 없다. 엔진 정리 경로도 이 항목을 짚지 못한다 — `APlayerController::Destroyed` 가 부르는 `ForceClearUnpauseDelegates(this)` 는 델리게이트의 UObject 가 그 PC 일 때만 매칭하는데(`GameModeBase.cpp:274-286`), 여기 `FCanUnpause`(150행)의 소유자는 서브시스템이라 `Pausers` 에 그대로 남고, 옛 PC 가 pauser 였으면 엔진이 정지 소유권을 새 PC 의 PlayerState 로 옮겨 정지가 유지된다. 실제로는 새 레이아웃에 위젯이 하나라도 push 되면 `ObserveWidgetForGamePause → RefreshGamePause → SetPause(false)` 가 돌아 `ClearPause` 가 남은 델리게이트를 회수하므로 대개 곧 풀린다. 아무것도 push 하지 않는 PC(레이아웃 클래스 미지정 등)로 갈아타면 화면엔 아무것도 없는데 월드만 정지된 채 남는다.
...
- **확신도**: 중간 (같은 월드 내 PC 교체 + 정지 위젯 열림 + 새 PC 가 아무것도 띄우지 않음이 겹쳐야 눈에 보인다. 맵 이동은 GameMode 가 새로 만들어져 영향이 없다)

### 5. 🟢 `UWxGamePopup::KillPopup` 은 호출자가 없는 데드 코드다
- **위치**: `Plugins/WxUI/Source/WxUI/Public/Widget/WxGamePopup.h:70`, `Plugins/WxUI/Source/WxUI/Private/Widget/WxGamePopup.cpp:91-93`, `Plugins/WxUI/Source/WxUI/Private/Widget/WxConfirmationPopup.cpp:80-84`
- **범주**: 중복/복잡도
...
- **미검토 / 한계**: WBP 내부(위젯 계층·MVVM 바인딩 행·이벤트 그래프)는 범위 밖이다. `AWxIndicator::UpdateProjection` 의 화면 밖 접힘·역투영 수식은 로직을 따라 읽었으나 실행 검증은 하지 않았고, 성능은 코드 판독 기준이며 프로파일링하지 않았다(네임플레이트의 프레임당 `UGameplayStatics::GetPlayerPawn`, 인디케이터의 프레임당 행렬 역변환은 규모상 문제로 보지 않아 발견에 넣지 않았다). 발견 3 은 두 위젯이 실제로 어느 z-order 로 뷰포트에 붙는지까지는 확인하지 않았다. 아래는 이번에 확인해 보고 발견으로 올리지 않은 지점이라, 다음 리뷰가 같은 자리를 다시 파지 않도록 남긴다.
  ...
  - 소스 57파일 전부의 첫 줄이 `// Copyright Woogle. All Rights Reserved.` 다(규칙 2 충족). 헤더 인라인 정의는 StateTree 두 노드의 `GetInstanceDataType()` 뿐이고 두 헤더 모두 예외 사유 주석을 달고 있다(규칙 6 충족).
  - 직전 리뷰가 규칙 위반으로 올렸던 `UWxViewModel_Ability::TryActivateAbility` 의 `BlueprintCallable`(`WxViewModel_Ability.h:41`)은 걷어냈다 — 뷰모델의 Command 함수는 승인된 예외이고, 실제로 `WBP_Ability`·`WBP_AttackSlot` 그래프가 이 함수를 직접 부른다. 남는 것은 `Request~` 작명 관례와의 불일치뿐이라 CLAUDE.md 위반이 아니다.
  - `UWxUILibrary` 의 `MakeNativeResultDelegate`(`WxUILibrary.cpp:14-28`)가 쓰는 `CreateWeakLambda` 는 동적→네이티브 델리게이트 변환이라 규칙 3 위반으로 올리지 않았다(`CreateUFunction` 로 대체 가능하긴 하다).
  ...
  - `UWxAsyncAction_PushWidgetToLayer` 의 동기 완료 재진입(`Activate` 22-60행, `HandleWidgetClassLoaded` 77-145행)은 `bFinished` 게이트로 모든 분기가 막혀 있다. `UWxViewModel::RequestImageAsync` 의 슬롯 재요청·맵 재해시 처리도 주석대로 방어돼 있다.
  - 레이아웃 부재·`LayoutClass` 미지정이 조용히 아무것도 안 하는 경로(`WxPlayerLayoutComponent.cpp:55-60`, `WxUIManagerSubsystem.cpp:341-353`)는 실패가 화면에서 즉시 보이는 계약이라 진단 가드를 제안하지 않았다.

---
*문서 기준 커밋 `53bc7de6` · 리뷰일 2026-09-08 · 소스 57파일 — `/module-review`로 갱신*
