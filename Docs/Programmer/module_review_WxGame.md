...
> Experience 부트스트랩·캐릭터 계층·MVVM 접착 모두 책임 경계가 또렷하고 주석이 설계 근거를 잘 남겨 전반적으로 건강하다. 🔴 는 없고, 남은 발견은 대부분 "싱글플레이 전제가 코드에 함정으로 남은 것"과 재진입·순서 멱등성 부족이다. 이번 리뷰는 프레임워크 8종·캐릭터 7종·컨트롤러 2종·어빌리티 2종·MVVM 10종의 cpp 까지 내려가 읽고 직전 리뷰(`13b45192`)의 항목을 현재 코드로 전수 재검증했다 — 그 사이 Experience 미확정 진단(당시 🟢 7번)은 고쳐졌고, `GiveAbilitySet` 중복 부여도 WxCombat 쪽 가드로 해소돼 2번의 범위가 줄었다.
...
- **범주**: 버그/정확성
- **문제**: 호출될 때마다 (1) `BaseWalkSpeed = MaxWalkSpeed` 를 다시 잡고(`:202`), (2) SPD 변경 델리게이트를 `AddUObject` 로 또 붙인다(`:207-208`). 진입점이 `PossessedBy`(서버, `:108`)와 `OnRep_PlayerState`(클라, `WxPlayerCharacter.cpp:64`) 두 곳이라, 같은 폰에서 두 번 불리면 SPD 배율이 이미 곱해진 속도가 새 기준값이 되어 배율이 복리로 누적된다. `GiveAbilitySet` 은 이제 `bAbilitySetGranted` 가드가 있어(`Plugins/WxCombat/Source/WxCombat/Private/AbilitySystem/WxAbilitySystemComponent.cpp:18-28`) 이중 부여는 해소됐지만, 기준 속도 재캡처는 그대로 남아 있다. 현재 같은 폰을 재빙의하는 호출이 없어 잠복 상태지만, 리스폰·컨트롤러 스왑을 도입하는 순간 터진다. 부수적으로 `BaseWalkSpeed` 는 헤더에서 초기화되지 않고(`WxCharacterBase.h:124`), 바인딩 직후 현재 SPD 를 1회 적용하지 않아 바인딩 전에 이미 배율이 복제돼 있던 클라 폰은 다음 변경까지 기준 속도로 남는다.
...
- **위치**: `Source/WxGame/Character/WxEnemyCharacter.cpp:78-82`
- **범주**: 설계/구조
- **문제**: `HandleDeath` 가 권위에서 `UGameplayStatics::GetPlayerController(this, 0)` 를 보상 수령자로 쓴다. 리슨/데디케이티드 서버에서 0번은 "호스트 또는 첫 접속자"이지 처치자가 아니므로, 다른 플레이어가 잡은 적의 재화·픽업이 엉뚱한 인벤토리로 들어간다. 게다가 최근 리팩터로 `AWxEnemyCharacter` 의 대미지 구독이 걷혔고(반응 처리는 `UWxAIPerceptionComponent` 의 `Event.Hit` 구독으로 이동) 사망 이벤트의 `Instigator` 는 죽은 본인으로 채워지므로(`Plugins/WxCombat/Source/WxCombat/Private/AbilitySystem/Attribute/WxCombatAttributeSet.cpp:116-117`), 지금은 처치자를 되짚을 재료조차 이 클래스에 남아 있지 않다. 같은 0번 플레이어 가정이 백스탭 판정(`IsLocalPlayerInRearCone`, `:43-61`)에도 있으나 그쪽은 헤더 주석(`WxEnemyCharacter.h:47-48`)이 한계를 명시해 둔 상태다.
- **제안**: 마지막 가해자를 남기려면 대미지 GE 컨텍스트의 Instigator 를 사망 이벤트 페이로드까지 전달하도록 WxCombat 쪽을 손봐야 한다. 싱글 전용으로 확정한 설계라면 최소한 보상 쪽에도 백스탭 판정처럼 "MP 비대응" 을 주석에 명시해 다음 사람이 함정에 빠지지 않게 한다.
- **확신도**: 중간
...
- **범주**: 중복/복잡도
- **문제**: `UWxMetaHumanComponent::OnRegister` 는 오너를 `Cast<ACharacter>` 해 `GetMesh()` 를 리더 메시로 잡고, 실패하면 조용히 반환한다(`WxMetaHumanComponent.cpp:36-41`). `AWxNpc` 의 베이스 `AWxDialogueActor` 는 `AActor` 직상속이라(`Plugins/WxDialogue/Source/WxDialogue/Public/WxDialogueActor.h:19`) 이 캐스트가 항상 실패하고, 생성자에서 붙이는 `MetaHumanComponent`(`WxNpc.cpp:37`)는 어떤 슬롯을 채워도 바디·페이스·그룸을 만들지 않는다. 헤더 주석(`WxNpc.h:17`)은 "외형 컴포넌트를 얹는 합성"이 이 클래스의 역할이라고 적어 기획자가 BP_Npc 슬롯을 채우면 동작할 것처럼 읽히는데, 코드에 경고조차 없어 채우는 순간 무증상 실패가 된다. `AWxNpc` 는 자기 `MeshComponent`(`WxNpc.cpp:26`)를 갖고 있으므로 리더로 쓸 메시 자체는 존재한다.
...
- **미검토 / 한계**:
  - 규칙 위반은 이번에도 발견이 없다: 모듈 전체에 `FORCEINLINE`·인라인 정의·람다가 없고, 66개 파일 전부 첫 줄 저작권 표기를 갖췄으며, `Wx` 접두사·`Handle` 접두사도 일관된다.
  - 뷰모델의 `BlueprintCallable` 명령 함수는 프로젝트가 규칙 예외로 확정한 사항이라 이번에도 제외했다. 어빌리티·이동 입력 콜백(`Move`·`Look`·`ToggleCrouch`·`AbilityInput*`)의 `Handle` 접두사 누락도 엔진 관례를 따른 의도로 보아 제외했다.
  - 도메인 플러그인은 이 모듈이 부르는 지점만 교차 확인했다(`UWxAbilitySystemComponent::GiveAbilitySet`·`GetAbilityInputActions`, `UWxCombatAttributeSet::ProcessDamageTaken`, `UWxHUDComponent`, `UWxUIManagerSubsystem`, `UWxViewModel` 베이스, `AWxDialogueActor` 의 상속) — 그 내부 정확성은 보지 않았다.
  ...
  - 클라의 시뮬레이션 프록시 적에는 `InitAbilitySystem` 이 돌지 않아 SPD 배율이 `MaxWalkSpeed` 에 반영되지 않지만, 이동 자체가 복제되고 `WxCharacterBase.cpp:61` 주석이 그 사이드 제한을 명시하고 있어 의도로 보아 발견에서 뺐다.
  - BP/WBP 의 실제 슬롯·바인딩 구성은 범위 밖이다.

---
*문서 기준 커밋 `cf3a7a0` · 리뷰일 2026-08-25 · 소스 66파일 — `/module-review`로 갱신*
