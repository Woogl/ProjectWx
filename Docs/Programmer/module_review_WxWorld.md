...
> 장치 상태 스냅샷 수렴·통보 기반 무틱 대기·스포너 수명이라는 세 축의 골격은 여전히 견고하고, 코딩·모듈 규칙 위반은 한 건도 없다(61개 소스 전수 확인). 직전 리뷰(`6ea7624`) 의 열 건을 현행 코드에서 전부 재확인했고 고쳐진 것은 없었으며, 저작 정책을 태스크별로 다시 대조하다가 'Play Animation' 이 복구·재선택 정책을 둘 다 세우지 않는 것을 새로 찾아 기존 10번과 한 건으로 묶었다. 장치 상태머신(`WxDeviceStateTreeComponent.cpp` 491줄)·상호작용 스캐너·스포너·대기 등록부와 16개 StateTree 태스크 구현을 모두 읽었고, 계약 상대인 `WxGame` 의 `AWxEnemyCharacter`·`UWxRespawnLibrary`·`UWxViewModel_InteractionList`·`AWxNpc` 와 엔진의 `FStateTreeInstanceData` 까지 교차 확인했다.
...
- **범주**: 버그/정확성
- **문제**: 지속형 연출 태스크 중 이 태스크만 생성자가 없고 `FWxDeviceExecutionPolicy` 도 부르지 않아, 두 축 모두에서 이웃과 다르게 돈다. (a) **복구**: `ComponentMove`(`Private/StateTreeTask/WxStateTreeTask_ComponentMove.cpp:37`)·`SplineMove`(`:50`)는 `IsRestoringDevice` 면 목표 포즈로 스냅하는데, `PlayAnimation` 은 무조건 처음부터 재생한다. 그래서 클라 복구·레이트조인·`InitialState` 시작에서 문이 열린 상태로 들어가면 슬라이드는 끝점으로 스냅하고 애니메이션만 열리는 연출을 처음부터 다시 돌아, 정확히 스냅샷 설계가 막으려던 어긋남이 생긴다. 게다가 헤더(`:33`)는 이 방침을 "'Component Move' 와 동일한 방침"이라 설명하는데 그 근거가 사실과 다르다 — `ComponentMove` 는 초기 시작에서도 스냅한다(`StartLogic` 이 `bRestoringState` 를 켠 채 진입시키므로, `Private/Device/WxDeviceStateTreeComponent.cpp:96`). (b) **재선택**: 위 셋과 `PlayLevelSequence`(`:16`)는 `bShouldStateChangeOnReselect = false` 를 세우는데 이 태스크만 기본값(true)이라, 클라 추종이 새 EntrySerial 로 같은 태그를 Critical 재요청해 상태를 재선택시키면(`Private/Device/WxDeviceStateTreeComponent.cpp:437`, 테스트 "New same-tag entry reselects once" `Private/Device/Tests/WxDeviceTests.cpp:311`) 애니메이션만 처음부터 다시 돌고 슬라이드는 이어진다.
...
- **위치**: `Plugins/WxWorld/Source/WxWorld/Private/Device/WxDeviceStateTreeComponent.cpp:195`, `Plugins/WxWorld/Source/WxWorld/Private/Device/WxDeviceStateTreeComponent.cpp:358`, `Plugins/WxWorld/Source/WxWorld/Private/Device/WxDeviceStateTreeComponent.cpp:450`
- **범주**: 설계/구조
- **문제**: `ObserveActiveState()` 는 `Execution->ActiveFrames` 전체를 역순으로 훑고 각 프레임의 `Frame.StateTree` 에서 상태를 꺼내므로(`:197`), 링크된 서브트리 에셋의 Tag 까지 `LastEnteredTag` 로 잡아 스냅샷에 실어 보낸다. 반면 `HasState()`·`RequestState()` 는 루트 `StateTreeRef.GetStateTree()` 한 곳에서만 핸들을 찾는다. 그래서 서브트리 태그가 발행되면 클라의 `FollowAuthorityState()` 가 `HasState(TargetTag)` 에서 걸려 `FailSynchronization("Authority tag is missing from the local root asset")` 로 빠지고(`:358`), `SyncFailure` 는 한 번 세워지면 지워지는 곳이 스냅샷 수신(`OnRep_StateSnapshot:290`)뿐이라 같은 태그가 계속 오는 동안 그 장치는 클라에서 영영 어긋난 채 남는다. 게다가 로그 문구가 "로컬 루트 에셋에 없다"여서 서버·클라 에셋 불일치로 오진하게 만든다.
...
- **교차 확인**: `Source/WxGame/Character/WxEnemyCharacter.cpp:111`(스폰 대상의 자가 부착 → 2번), `Source/WxGame/Framework/WxRespawnLibrary.cpp:78`(사망마다 `TryRespawnAll` 이 도는 것 → 2번), `Source/WxGame/MVVM/WxViewModel_InteractionList.cpp:82`(`RequestInteract` 가 폰 입력 스택을 타지 않음 → 5번), `Source/WxGame/Character/WxNpc.cpp:43`(`IsAwaited` 소비 → 6번), `Source/WxEditor/WxDeviceLinkVisualizer.h:12`(컴포넌트가 export 되지 않는다는 기록 → 8번), 엔진 `Engine/Plugins/Runtime/StateTree/Source/StateTreeModule/Public/StateTreeInstanceData.h:549`(`GetExecutionState()` 가 5.8 에서 항상 유효 포인터를 돌려주는 것을 확인 — `StartLogic`·`ObserveActiveState` 의 무가드 역참조는 문제가 아니다)
- **미검토 / 한계**: StateTree·장치 BP 에셋의 실제 Tag·전이 조립과 WBP 내부는 범위 밖이다. 3·4·5·6·10번은 실행 재현 없이 코드 경로로만 검토했으므로 실제 배치(애니메이션과 이동을 한 상태에 묶은 장치 유무, 링크 서브트리 사용 여부, 입력을 끄는 장치가 다른 상호작용 대상과 겹치는지, 원격 클라 세션 유무, 루프 FX 저작 여부)에서 재현 조건이 성립하지 않을 수 있다. 규칙 준수는 61개 소스 전체를 검색해 확인했다 — 첫 줄 저작권 전부 존재, `WxCore` 외 Wx 플러그인 참조 없음(`WxGameplayTags`·`WxInteractable`·`WxLocatorUtils` 모두 `WxCore` 소속이며 `.uplugin` 의존도 `WxCore` 하나), `Wx` prefix 준수, `BlueprintCallable` 은 `UWxSpawnerLibrary::TryRespawnAll` 한 곳뿐(Blueprint Function Library 라 적법), `FORCEINLINE`·헤더 인라인 정의 없음(`GetInstanceDataType()` 은 16개 파일 전부가 예외 사유 주석을 붙였고 `TWxStateTreeWaitRegistry` 도 템플릿 예외 사유를 명시했다), 델리게이트 바인딩 대상은 타이머 콜백 `HandleScanTimer` 하나이며 `Handle` prefix 를 지킨다. 람다는 스캐너의 거리 정렬 술어(`WxInteractionScannerComponent.cpp:182`) 하나로 캡처가 필요한 정당한 사용이다. `override` 는 `AWxSpawner::GetDefaultActorLabel`(라벨을 전면 대체하므로 `Super` 호출이 무의미)과 `UWxDeviceStateTreeComponent::IsRunning`(순정 값을 조건으로 그대로 쓴다) 외 전부 `Super` 를 부른다.

---
*문서 기준 커밋 `53bc7de6` · 리뷰일 2026-09-08 · 소스 61파일 — `/module-review`로 갱신*
