...
> 기믹의 세 책임(상호작용·영속·ST 구동)을 컴포넌트 하나로 접고 동작은 전부 ST 에셋이 author 하게 한 구조가 일관되며, 널 가드·권위 가드·초기진입/라이브 분기·차단 해제 짝맞춤이 성실하다 — 남은 문제는 「멀티플레이 수렴 경로」와 「에디터 데이터 부여 경로」, 그리고 아직 에셋에 실리지 않아 발현하지 않은 태스크 몇 개에 몰려 있다. 이번 리뷰는 C++ 18파일을 전부 열고 기믹 컴포넌트·기믹 ST 노드 라이브러리·스캐너·스포너·스포너 ST 노드의 cpp 를 정독했으며, 판정 근거로 UE 5.8 엔진의 `UStateTreeComponent`·`FStateTreeExecutionContext`·`FStateTreeInstanceStorage`·`AActor` 프로퍼티 플래그를 라인 단위로 대조하고, 소비처(`WxAbility_Interact`·`WxViewModel_InteractionList`·`AWxEnemyCharacter`)와 WxCore 계약도 함께 확인했다. 직전 리뷰(`18f580a2`)의 지적은 하나도 복사하지 않고 전부 현행 코드·엔진 라인으로 재검증했으며, 그 결과 해소된 것은 없어 유효한 것만 이월하고 새 발견 3건을 더했다.
...
### 1. 🔴 `OnRep_StateTag` 가 아직 소비되지 않은 대기 이벤트를 "어긋난 피어"로 오판해, 클라에서 트리거형 연출이 통째로 스킵된다
...
- **확신도**: 중간(엔진의 이벤트 큐잉·`InstanceData.Reset()`·`EventQueue->Reset()` 은 라인 단위로 확인. 다만 멀티캐스트 RPC 와 프로퍼티가 실제로 같은 네트 업데이트에 실리는지는 서버 프레임 내 「어빌리티 실행 vs 기믹 컴포넌트 틱 vs TickFlush」 순서에 달려 있어 미실측)
...
- **범주**: 버그/정확성
- **문제**: 두 가지가 겹쳐 있다.
  - **복제 충돌**: `SaveId` 를 `PostActorCreated`/`PostDuplicate` 두 훅에서만 심는데, 에디터의 액터 복제(Ctrl+D·Alt-드래그·복사/붙여넣기)는 `StaticDuplicateObject` 가 아니라 텍스트 export/import 경로다 — 새 액터를 스폰한 뒤(`PostActorCreated` 통과) 원본 T3D 를 import 로 덮어쓰므로, 플래그 없는 평범한 `UPROPERTY` 인 `SaveId`(`WxSpawner.h:82-84`)는 **원본 값으로 되돌아가고** `PostDuplicate` 는 불리지 않아 교정 기회도 없다. 반면 `ActorGuid` 는 엔진에서 `NonPIEDuplicateTransient, TextExportTransient` 로 선언돼(엔진 `GameFramework/Actor.h:1096-1097`) T3D 에 실리지 않고 스폰 시 발급된 새 값을 유지한다. 결과적으로 복제한 스포너 둘이 같은 `SaveId` 로 한 슬롯 레코드를 공유하므로, 한쪽 보스를 잡으면 다른 쪽도 처치 상태로 복원된다(`bIsKilled` 는 `SaveGame`, `WxSpawner.h:78-80`).
...
- **문제**: `SetTimer(..., &UWxInteractionScannerComponent::ScanAndPush, ...)` 로 타이머 델리게이트에 직접 바인딩된다. 모듈 안에는 다른 사례가 없지만, 프로젝트의 다른 타이머 콜백은 모두 규칙을 지킨다 — `UWxAbility_Groggy::HandleMontagePollTick`·`HandleGroggySafetyTimeout`(`Plugins/WxCombat/Source/WxCombat/Private/AbilitySystem/Ability/WxAbility_Groggy.cpp:71-73`).
