...
> FastArray 기반 서버 권위 인벤토리로, 모듈 경계(`WxCore` 외 의존 0)와 코딩 규칙을 전부 지키고 리플리케이션 콜백의 델타 계산 의도가 주석에 잘 남아 있는 건강한 모듈이다. 이번 리뷰는 `Build.cs`/`uplugin`과 공개 헤더 전부, 그리고 인벤토리 컴포넌트·아이템 인스턴스·장비 컴포넌트·픽업·보상 라이브러리·StateTree 태스크의 cpp 로직을 읽고, 외부 호출부(`Source/WxGame` 의 어빌리티·아이템 사용 컴포넌트·뷰모델·적 캐릭터)까지 교차 확인했다.
...
- **범주**: 버그/정확성
- **문제**: `PostReplicatedAdd` 는 `Entry.LastObservedCount = Entry.StackCount`(`:83`)를 **무조건 먼저** 수행하고, 그다음 `if (Entry.Instance)`(`:85`)로 통지 여부를 가른다. `UWxItemInstance` 는 `bReplicateUsingRegisteredSubObjectList` 로 복제되는 동적 서브오브젝트라, 신규 엔트리의 `Instance` 참조가 같은 번치 안에서 아직 생성되지 않은 오브젝트를 가리켜 클라에서 null 로 들어올 수 있다. 이때 통지는 건너뛰는데 `LastObservedCount` 는 이미 최신값이라, 이후 참조가 해석되어 `PostReplicatedChange` 가 와도 `Delta = StackCount - LastObservedCount == 0`(`:104`) 이라 `:107` 조건에서 또 걸러진다 — **해당 슬롯의 획득 통지가 영구 유실**된다. 인벤토리 목록의 유일한 소비자인 `UWxViewModel_Inventory` 는 `OnInventoryStackChanged` 를 받아야만 `RefreshAllItems()` 를 돌리므로(`Source/WxGame/MVVM/WxViewModel_Inventory.cpp:115`), 데디케이티드 클라에서 픽업한 아이템이 UI에 끝내 안 뜨는 형태로 드러난다. 같은 배치 안에서 `NotifyStackChangedFromList` 가 부르는 `GetTotalItemCountByDefinition`(`:619`) 도 미해석 엔트리를 건너뛰므로 `NewCount` 자체가 과소 집계된다.
...
- **확신도**: 높음 (다만 현재 호출부는 모두 스스로 권위를 가리고 있어 — `UWxItemUseComponent::HandleUseItemEvent`(`Source/WxGame/Inventory/WxItemUseComponent.cpp:74`), `UWxAbility_Interact` 의 `ServerOnly` + `HasAuthority` 이중 게이트, StateTree 두 태스크의 `Owner->HasAuthority()` — 즉시 발현되지는 않는다)
...
- **범주**: 버그/정확성
- **문제**: 머지 루프는 `InventoryList.GetEntries()` 로 내부 `Entries` 배열 참조를 잡은 채(`:278`) 매 반복마다 `NotifySlotChangedFromList`/`NotifyStackChangedFromList` 로 외부 구독자를 호출한다(`:295`~`:296`). 구독자가 그 콜백 안에서 인벤토리를 변경하면(획득 즉시 자동 사용·변환, 퀘스트 훅 등) 엔트리가 추가·삭제되어 `EntryIndex` 가 다른 슬롯을 가리키게 되고, 다음 반복의 `AddToEntryStack(EntryIndex, ...)` 이 엉뚱한 슬롯에 수량을 얹는다 — `AddToEntryStack` 은 인덱스 유효성도 검사하지 않는다(`:160`). 현재 구독자(`UWxViewModel_Inventory`, `UWxViewModel_Item`)는 읽기 전용이라 발현되지 않지만, 델리게이트가 public 이라 구독자 추가만으로 조용히 깨진다.
...
- **확신도**: 낮음 (의도된 설계일 수 있음 — 두 태스크의 헤더 주석이 "로컬 플레이어(0번 컨트롤러)"를 명시적 전제로 적어두었고, `AWxEnemyCharacter::HandleDeath` 의 처치 보상도 같은 방식을 쓴다(`Source/WxGame/Character/WxEnemyCharacter.cpp:180`). 즉 모듈 단독 결함이 아니라 프로젝트 전반의 싱글플레이 전제다)
...
### 9. 🟢 `AWxItemPickup::OnSaveRestored` 가 `SetItemDef` 본문을 복제하고, 그 값은 클라에 전달되지 않는다
- **위치**: `Plugins/WxInventory/Source/WxInventory/Private/Items/WxItemPickup.cpp:131`~`:133` (원본 `:52`~`:57`)
- **범주**: 중복/복잡도
- **문제**: `OnSaveRestored` 가 `ItemDef` 대입 + `FMath::Max(1, ...)` 클램프 + `ApplyPickupVisual()` 이라는 `SetItemDef` 의 본문 3줄을 그대로 다시 쓴다. 한쪽만 고치면 어긋나는 전형적 중복이다. 덧붙여 `ItemDef`/`Quantity` 는 `COND_InitialOnly` 로 복제되므로(`WxItemPickup.h:48`~`:49`), 복원이 액터의 초기 번치 이후에 일어나는 순서라면 클라는 복원된 값을 영영 못 받는다 — 스폰 직후 주입을 전제로 한 조건이 복원 경로에는 맞지 않는다.
- **제안**: `OnSaveRestored` 는 `SetItemDef(PersistedState.ItemDefinition.LoadSynchronous(), PersistedState.Quantity)` 한 줄로 대체한다. 복제 조건은 복원 시점이 초기 번치보다 늦을 수 있는지 확인한 뒤, 늦다면 `COND_None` 으로 바꾸거나 `SetItemDef` 에서 `ForceNetUpdate` 를 건다.
- **확신도**: 중간 (중복 자체는 확정이나, 복제 조건 문제는 LSP 복원 타이밍을 실측하지 않아 발현 여부가 미확인이다)
