...
> 모듈 내부 코드 품질 자체는 여전히 좋다. 권위 게이팅이 전 변경 진입점에 빠짐없이 걸려 있고, 서버 변경 경로와 클라 복제 콜백이 같은 `Notify*` 진입점으로 수렴하는 구조도 일관된다. **심각(🔴) 등급은 없다** — 이 문서의 최초 작성분이 "아이템 인스턴스 서브오브젝트가 리모트 클라로 복제되지 않는다"를 🔴로 올렸으나, 엔진 5.8 소스 재확인 결과 **오탐이어서 철회했다**(근거는 아래 「미검토/한계」 (a)). 남은 8건은 지난 리뷰(`14a77aef`)에서 지적된 것들이며 그 사이 커밋이 문서·주석 정리라 전부 그대로 남아 있다. 이번 리뷰는 소스 22개를 모두 열고 `WxInventoryManagerComponent.cpp`·`WxItemPickup.cpp`·`WxEquipmentComponent.cpp`·`WxRewardLibrary.cpp`·StateTree Task 2종을 라인 단위로 본 뒤, 엔진 5.8 소스(`DataChannel.cpp`·`ActorReplication.cpp`·`ActorComponent.cpp`·`PackageMapClient.cpp`·`Actor.cpp`), 호출부(`Source/WxGame` 의 UseItem/Interact 어빌리티·뷰모델), 에셋 덤프(`BP_ItemPickup`·`BP_PlayerController`)로 교차 검증했다.
...
- **범주**: 버그/정확성
- **문제**: `LaunchInDirection` 은 `HasAuthority()` 게이트 뒤에서 서버만 `SetSimulatePhysics(true)` + `SetPhysicsLinearVelocity` 를 걸고(`:58-65`), 생성자는 `bReplicates = true`(`:21`) 만 켠다. `AActor::bReplicateMovement` 는 엔진 기본값이 false 다 — UE 5.8 `AActor::InitializeDefaults`(`Actor.cpp:275-`)는 이 비트를 건드리지 않고 `APawn` 만 생성자에서 `SetReplicatingMovement(true)` 를 부른다(`Pawn.cpp:89`). 파생 BP 도 마찬가지다(`.claude/asset_dump/Blueprints/BP_ItemPickup.json:43` 이 `"bReplicateMovement": false`). 결과적으로 서버에서만 픽업이 굴러가고 클라 화면에서는 스폰 지점에 그대로 떠 있다. 상호작용의 사거리 검증은 서버 위치 기준(`Source/WxGame/AbilitySystem/Ability/WxAbility_Interact.cpp:85`)이라, 보이는 자리에서는 F 가 안 먹고 아무것도 없는 자리에서 프롬프트가 뜨는 불일치가 된다. 같은 액터가 `ItemDef`/`Quantity` 를 `COND_InitialOnly` 로 세심하게 복제하고 있어(`:45-46`) 멀티 인지 자체는 있는 코드다.
...
- **위치**: `Plugins/WxInventory/Source/WxInventory/Private/Items/WxItemPickup.cpp:53` (`SetItemDef` → `ApplyPickupVisual`), 실제 로드는 `:132`, `:137`, 활성화는 `:140`
- **범주**: 성능/안전
...
- **제안**: 한 줄 조기 반환을 넣거나, 저ROI 가드로 판단해 넣지 않을 거라면 그 판단을 주석으로 남긴다 — 근거 없이 두면 리뷰마다 같은 항목이 다시 올라온다(이번이 네 번째 지적이다).
- **확신도**: 낮음
...
- **훑은 파일**: `Plugins/WxInventory/Source/WxInventory/Public/Items/WxItemFragment.h`, `Plugins/WxInventory/Source/WxInventory/Private/Items/WxItemFragment.cpp`, `Plugins/WxInventory/Source/WxInventory/Public/Items/WxItemDefinition.h`, `Plugins/WxInventory/Source/WxInventory/Private/Items/WxItemDefinition.cpp`, `Plugins/WxInventory/Source/WxInventory/Public/Items/WxRewardTableRow.h`, `Plugins/WxInventory/Source/WxInventory/Private/Items/WxRewardTableRow.cpp`, `Plugins/WxInventory/Source/WxInventory/Public/Items/WxItemPickup.h`, `Plugins/WxInventory/Source/WxInventory/Public/Items/WxItemInstance.h`, `Plugins/WxInventory/Source/WxInventory/Public/Inventory/WxEquipmentComponent.h`, `Plugins/WxInventory/Source/WxInventory/Public/Inventory/WxRewardStateTreeNodes.h`, `Plugins/WxInventory/Source/WxInventory/Public/Inventory/WxInventoryStateTreeNodes.h`, `Plugins/WxInventory/Source/WxInventory/Public/WxRewardLibrary.h`, `Plugins/WxInventory/Source/WxInventory/Public/WxInventoryModule.h`, `Plugins/WxInventory/Source/WxInventory/Private/WxInventoryModule.cpp`, `Plugins/WxInventory/Source/WxInventory/WxInventory.Build.cs`, `Plugins/WxInventory/WxInventory.uplugin`, 경계 확인용 외부 호출부(`Source/WxGame/AbilitySystem/Ability/WxAbility_UseItem.cpp`, `WxAbility_Interact.cpp`, `Source/WxGame/MVVM/WxViewModel_Item.cpp`, `WxViewModel_Inventory.cpp`, `Source/WxGame/Controller/WxPlayerController.cpp`)
- **미검토 / 한계**: 런타임 검증(PIE 2인 이상 네트워크 테스트)은 하지 않았다. #1·#2 는 엔진 소스와 에셋 덤프 기반 정적 판단이라 실제 재현은 남아 있다. BP/WBP 내부와 ItemDefinition 데이터 자산의 Fragment 조합 타당성은 범위 밖이다. 반대로 이번에 확인해 **문제없다고 판단한 것**은 다음 세션이 중복 조사하지 않도록 남긴다 — (a) **서브오브젝트 복제는 정상이다(🔴 오탐 철회, 재조사 금지)**: 이 문서의 최초 작성분은 "컴포넌트만 `bReplicateUsingRegisteredSubObjectList`(`WxInventoryManagerComponent.cpp:228`)를 켜고 소유 `AWxPlayerController` 는 껐으므로 `UWxItemInstance` 가 리모트 클라로 복제되지 않는다"고 🔴 로 올렸으나, **엔진이 정확히 이 조합을 지원한다**. 레거시 경로 `AActor::ReplicateSubobjects`(`ActorReplication.cpp:592-625`)는 각 복제 컴포넌트에 대해 가상 함수 `ActorComp->ReplicateSubobjects(...)`(베이스는 `false` 반환) **뿐 아니라 그 직후 `Channel->ReplicateSubobject(ActorComp, *Bunch, *RepFlags)` 도 호출한다**. 이 오버로드(`DataChannel.cpp:4346`)의 주석이 "This traps actor components using the registration list even if their owning Actor isn't using the list itself" 이고, 내부의 `checkf(Actor->IsUsingRegisteredSubObjectList() == false, ...)`(`:4364`)는 **액터가 플래그를 끈 상태가 이 분기의 전제**임을 못박는다. 이어 `WriteComponentSubObjects`(`:4391`)가 `GetSubObjectsOfActorComponent(Actor, ReplicatedComponent)` 로 컴포넌트 레지스트리를 꺼내 `WriteSubObjects` 로 전부 채널에 쓴다. `AddActorComponentReplicatedSubObject` 가 액터 플래그를 검사하지 않는 것도 버그가 아니라 이 설계의 귀결이다. 따라서 `AWxPlayerController` 에 플래그를 켜거나 `ReplicateSubobjects` 를 오버라이드할 필요가 **없다**. (b) **FastArray 참조 해결 순서도 안전하다**: `UActorChannel::ReplicateRegisteredSubObjects`(`DataChannel.cpp:4179-`)는 컴포넌트의 서브오브젝트를 컴포넌트 프로퍼티보다 **먼저** 쓴다("SubObjects have to be created before the component on the receiving end"). `PostReplicatedAdd` 시점에 `Entry.Instance` 는 유효하고, `LastObservedCount` 를 널 가드보다 앞서 갱신하는 것도 무해하다. (c) 모듈 경계: Build.cs/uplugin 의 Wx 의존은 `WxCore` 뿐이고 소스 내 비-WxInventory Wx 인클루드도 `WxInteractable.h`/`WxGameplayTags.h`/`WxCollisionChannels.h`(전부 WxCore) 뿐이다. (d) 규칙 준수: 22개 소스 전부 첫 줄 저작권 표기 존재, 람다·`FORCEINLINE` 0건, `BlueprintCallable` 은 `UWxRewardLibrary::GrantReward`(BP Function Library) 한 곳뿐, Wx prefix 누락 없음, 델리게이트 바인딩 자체가 모듈 내에 없어 `Handle` 규칙 대상 없음. 헤더 인라인은 템플릿 `FindFragmentByClass<T>()` 와 StateTree 관용 `GetInstanceDataType()` 뿐이며 둘 다 예외 사유가 주석에 있다. (e) FastArray 정합성: `AddEntry`/`RemoveEntry`/`AddToEntryStack`/`ConsumeByDefinition` 의 `MarkItemDirty`/`MarkArrayDirty` 조합, `CreateIterator`+`RemoveCurrent` 순회, `LastObservedCount` 의 `NotReplicated` 모두 올바르다. `FWxInventoryList(this)` 생성자 주입도 안전하다 — 네이티브 클래스 소유 프로퍼티는 `PostConstructLink` 에 들어가지 않아 CDO 값이 덮어쓰지 않는다. (f) 통지 수렴: 클라 `PreReplicatedRemove` 가 제거 대상 `StackCount` 를 0으로 내린 뒤 총량을 재계산하므로 서버/클라 최종 합계가 일치한다. (g) 권위: `AddItemDefinition`/`ConsumeItemsByDefinition`/`UseItemByDef`/`RefillItemCharges` 의 `check(HasAuthority())` 는 모든 호출 경로(서버 전용 `OnInteracted`, ServerOnly 상호작용 어빌리티, UseItem 어빌리티의 `HasAuthority` 게이트, 권위 게이트된 StateTree Task)에서 안전하다.

---
*문서 기준 커밋 `1e9b745c` · 리뷰일 2026-08-05 · 소스 22파일 — `/module-review`로 갱신*
