...
- **시점 축** — **라이브 전이**(상호작용 발동: `SourceStateID` 유효) / **초기·복원 진입**(ST 시작·세이브 복원·레이트조인: `SourceStateID` 무효 또는 `Gimmick.Restore` 마커)
...
- 태그 어휘는 WxCore 의 `Gimmick.<기믹>.<상태>`. 비기본 상태마다 그 태그를 *Required Event to Enter* 로 건다.
- 기본(resting) 상태(Idle/Close/Closed/Active 등)는 *Required Event 없이* 시작 시 선택되게 둔다.
- **불일치는 침묵 무동작**이며 컴파일이 못 잡는다(→ [검증 불가능한 계약](#검증-불가능한-계약)). 상태를 추가할 땐 **코드 태그(WxCore)와 에셋 전이를 한 커밋에서 함께** 바꾼다.
...
- 판정 헬퍼: `IsInitialOrRestoreEntry` = `!Transition.SourceStateID.IsValid() || Context.HasEventToProcess(Gimmick.Restore)`.
- 베이스가 `BeginPlay`/`OnWxSaveRestored` 에서 상태 태그를 **`Gimmick.Restore` 마커와 함께** 재발행해, 노드가 이 진입을 라이브 발동이 아닌 복원으로 보게 한다.
...
flowchart TD
    ...
    Send --> Enter["② ST: 그 태그를 Required Event 로 가진 상태 진입"]
    ...
    Restore["BeginPlay / OnWxSaveRestored"] -->|"State + Gimmick.Restore 마커"| Send
```
...
- **규칙 2 — 태그↔에셋 1:1.** State 태그가 어떤 ST 상태의 *Required Event* 와 정확히 일치해야 한다. 오타·전이 누락은 "조용히 안 됨"으로 나타난다(컴파일·런타임 모두 침묵). 다만 실패는 **테스트에서 기믹이 무반응으로 즉시 드러나므로**, 진단용 런타임 경고/에디터 검증 코드는 비용 대비 효용이 낮다고 보아 도입하지 않는다(필요 시 후속). 운용 규칙으로 대신한다: 상태 추가는 코드 태그와 에셋 전이를 한 커밋에서 함께 바꾼다.
