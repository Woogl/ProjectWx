...
> 소스 15개짜리 소형 모듈로, 러너 소유·권위·재진입·에셋 불가지라는 설계 전제가 doc-comment 에 명시돼 있고 코드가 그 전제를 지킨다 — 심각 결함은 없고 남은 문제는 대부분 "조용한 실패"(진단 부재·복구 불가 상태) 계열이다. 이번 리뷰는 `WxQuest.uplugin`·`WxQuest.Build.cs`와 Public/Private 전 소스 15개를 읽고, 소비자(`Source/WxGame/MVVM/WxViewModel_Quest.cpp`)와 **UE 5.8 엔진 소스(StateTree/GameplayStateTree)** 를 대조해 아래 발견 1·4 가 의존하는 엔진 동작을 소스로 확증했다.
...
- **위치**: `Plugins/WxQuest/Source/WxQuest/Private/Quest/WxQuestLibrary.cpp:16`, `Plugins/WxQuest/Source/WxQuest/Public/Quest/WxQuestLibrary.h:11-24`
- **범주**: 설계/구조
- **문제**: 컴포넌트는 재진입에 취약한 `ActivateQuest` 와 안전한 `RequestActivateQuest`(다음 틱 예약) 두 진입점을 갖는데(`WxQuestComponent.h:54-58`), BP 에 노출된 유일한 노드 `UWxQuestLibrary::StartQuest` 는 `ActivateQuest` 로 직결된다. "ST 실행 콜스택 밖에서만 호출"이라는 규약이 `WxQuestComponent.h:54` 의 주석으로만 존재하고, 정작 BP 사용자가 보는 `WxQuestLibrary.h:11-15` 의 doc-comment 에는 그 경고가 없다.

  이 노드가 러너 콜스택 안(퀘스트 ST 노드가 부르는 BP, ST 로 구동되는 기믹/대화 등)에서 한 번이라도 불리면 **복구 불가 상태**가 된다. UE 5.8 소스로 확인한 순서다.
  1. `StopLogic` → 재진입 컨텍스트로 `Context.Stop()`(`StateTreeComponent.cpp:246-249`) → `FStateTreeExecutionContext::Stop` 이 지연 처리되며 `Running` 을 반환한다(`StateTreeExecutionContext.cpp:1708-1714`). 반환값이 이전 상태와 같으니 `OnStateTreeRunStatusChanged` 는 이 시점에 발화하지 않는다(`StateTreeComponent.cpp:226-229`).
  2. `SetStateTreeReference` 는 실행 상태가 아직 `Running` 이라 **거부**된다("Trying to change the state tree on a running instance", `StateTreeComponent.cpp:497-500`).
  3. `StartLogic` → `StartTree` 도 `CurrentlyRunningExecContext` 가 살아 있어 **거부**된다("Reentrant call ... is not allowed", `StateTreeComponent.cpp:183`).
  4. 진행 중이던 `TickComponent` 가 끝나며 지연된 Stop 이 실행되어 `Stopped` 가 브로드캐스트되고(`StateTreeComponent.cpp:147-150`) `HandleStateTreeRunStatusChanged` → `ClearJournal` 로 이어진다.
...
- **확신도**: 높음 (엔진 동작은 소스로 확증. 실제로 콜스택 안에서 호출하는 에셋이 있는지는 BP/에셋 영역이라 미확인)
...
- **범주**: 버그/정확성
- **문제**: 모듈 README 가 못박은 태스크 규약은 "퀘스트 컴포넌트를 못 찾으면 잘못된 조립으로 보고 경고 후 노옵"인데(`Plugins/WxQuest/README.md:28`), 이 태스크만 `UE_LOG` 없이 `Failed` 만 반환한다. 형제 태스크 둘은 같은 상황에서 경고를 남긴다(`WxStateTreeTask_SetQuestTitle.cpp:26-28`, `WxStateTreeTask_SetQuestObjective.cpp:26-28`). 4종 중 이 파일만 `WxQuestModule.h` 를 include 하지 않은 것(`:3-8`)이 누락의 물증이다.

  "그럼 `Failed` 반환으로 드러나지 않는가"는 엔진 소스로 확인했다. UE 5.8 자동화 테스트 `System.StateTree.TasksCompletion.IneligibleTaskEnterStateFail`(`StateTreeTaskStateTest.cpp:956`, 단언 `:1001`)이 `bConsideredForCompletion=false` 인 태스크의 `EnterState` 실패는 **상태 완료 판정에서 무시된다**("ineligible EnterState failure is ignored")고 명시한다. 즉 세 태스크가 반환하는 `EStateTreeRunStatus::Failed`(`SetQuestTitle.cpp:28`, `SetQuestObjective.cpp:28`, `StartNextQuest.cpp:26`)는 전부 죽은 값이고, 조립 오류를 알릴 수단은 로그밖에 없는데 체인 태스크에만 그 로그가 없다. 실제 증상은 "퀘스트 체인이 다음으로 안 넘어가고 끝남"이며, 체인 종점(빈 지정)과 조립 오류가 겉으로 구분되지 않아 원인 추적이 불가능하다.
- **제안**: 형제와 같은 형식의 `UE_LOG(LogWxQuest, Warning, ...)` 을 추가하고 `WxQuestModule.h` 를 include 한다. 아울러 세 태스크의 doc-comment 에 "완료 판정에서 빠져 있으므로 반환값은 무시되고 진단은 로그가 유일하다"를 명시해 다음 사람이 `Failed` 를 신호로 오해하지 않게 한다 — 특히 `WxStateTreeTask_StartNextQuest.h:27` 은 현재 `Failed` 를 마치 유효한 종료 신호처럼 서술한다.
- **확신도**: 높음
...
- **문제**: `BeginPlay` 가 `UStateTreeComponent` 를 **GameState 를 Outer 로** 만들어 등록하는데(`:117-120`), 이 파일에는 `EndPlay`·`OnUnregister` 가 없다. 러너의 Outer 가 퀘스트 컴포넌트가 아니므로, 퀘스트 컴포넌트만 파괴되면 러너는 등록·틱 상태 그대로 남아 퀘스트 StateTree 를 계속 돌린다 — 저널을 갱신할 상대는 사라졌는데 월드 부수효과만 계속 나는 상태다. 재주입 시에는 `NewObject` 가 고정 이름 `TEXT("QuestStateTree")` 로 두 번째를 만들려다 살아 있는 기존 오브젝트와 충돌한다(`:117`).
  ...
  컴포넌트 단독 파괴 경로는 실재한다 — GameFeature 비활성화 시 주입 핸들(`TArray<TSharedPtr<FComponentRequestHandle>>`)을 놓으면 주입 컴포넌트가 걷혀 간다(`Source/WxGame/Framework/WxGameFeatureAction_AddComponents.h`, `.cpp`). 다만 현재 그 시점은 사실상 월드 종료와 묶여 있어 잠복 상태이며, 런타임 Experience 교체나 GameFeature 개별 토글이 생기는 순간 드러난다.
...
- **범주**: 설계/구조
- **문제**: `QuestTitle`·`Objectives`·`bHasActiveQuest` 는 복제 지정이 없고 `GetLifetimeReplicatedProps` 도 없다. 러너가 권위에만 생성되므로(`:110-121`) 리모트 클라이언트의 컴포넌트 사본은 저널이 영원히 빈 채로 남는다. 소비자 VM 은 로컬 컴포넌트에서 값을 pull 하므로(`Source/WxGame/MVVM/WxViewModel_Quest.cpp:41-52`) 클라이언트 HUD 에는 퀘스트가 표시되지 않는다. 클래스 주석(`WxQuestComponent.h:36`)이 "싱글/리슨 호스트" 전제를 명시하고 있어 의도된 v1 범위로 읽히나, 멀티플레이 마일스톤에서 반드시 되돌아와야 할 지점이므로 목록에 남긴다.
