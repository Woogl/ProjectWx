...
// CustomizeHeader 는 ST 인스턴스 데이터 직속 행에서 버려질 행에 한 번 더 돌므로, 호출마다 위젯과 멤버를 통째로 다시 만들어야 한다.
// 엔진 기본 UOL 편집기가 그 행에서 값 위젯을 못 그리는 것도 이 이중 호출 탓이다.
...
private:
	...
	/** null 이면 Clear 로 동작하며, 트랜잭션·변경 통지를 포함한다. */
