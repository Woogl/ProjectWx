 ...
 * 인벤토리·상호작용 스캐너·대화 세션·레이아웃 컴포넌트를 기본 서브오브젝트로 소유한다 — 폰 리스폰에도 살아남아야 하는 플레이어 단위 상태다.
 * 프론트엔드 컨트롤러에도 같은 컴포넌트가 붙지만 인벤토리는 비어 있고 스캐너·대화는 자기 가드로 무동작이다. 전투 HUD는 레이아웃 컴포넌트에서 지정하고, 프론트엔드 메뉴는 컨트롤러 BP에서 Push한다.
 */
UCLASS()
