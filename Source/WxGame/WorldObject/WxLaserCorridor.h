 ...
 *
 * 레이저 벽 BP(AWxEffectZone 파생)는 ST_LaserCorridor 의 Wx Spawn Actor 에 author 되어 SpawnPoint
 * 컴포넌트의 트랜스폼(위치·회전·스케일)에서 스폰되고, LaserLifetime 후 자동 파괴된다. SpawnPoint 를 통로
 * 입구에 배치하고 벽 크기에 맞춰 스케일하며, 액터 +X(ForwardVector) 방향이 전진 방향이다.
 *
 * 다른 기믹과 동일한 패턴: 권위 State(Active/Disabled)만 C++ 가 소유·확정하며(복제·SaveGame),
 * 스폰·활성 레이저 철거·인터랙션 토글은 전부 GimmickStateTree(ST_LaserCorridor)가
 * State 태그 이벤트로 진입한 상태에서 적용한다(Wx Spawn Actor / Wx Enable Interaction).
 *
...
protected:
	...
	// VisibleAnywhere + AllowPrivateAccess: StateTree 의 Wx Spawn Actor 가 SpawnPoint 로 바인딩하기 위한 노출. 통로 입구에 배치하고 벽 크기에 맞춰 스케일한다.
	...
	/** 스폰된 레이저 벽의 수명(초). 통로를 한 번 주파할 시간으로 맞춘다. Wx Spawn Actor 의 Lifetime 이 바인딩한다. */
