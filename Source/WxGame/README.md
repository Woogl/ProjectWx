...
- 게임 조립: GameMode BP(GM_FrontEnd·GM_Combat)가 폰·컨트롤러를 고르고, 컨트롤러·GameState 생성자가 도메인 컴포넌트를 기본 서브오브젝트로 붙인다.
...
| 타입 | 역할 | 위치 |
| --- | --- | --- |
| `AWxGameMode` | 폰 클래스 선택(FrontEnd 선택 → DefaultPawnClass)과 도착 검증. 맵별 구성은 BP 와 GameModeOverride | `Source/WxGame/Framework/WxGameMode.h` |
...
| `UWxGameFlowSubsystem` | 프론트엔드 선택(폰·목적지) 보관, 맵 전환, 도착 시 지형 확보 | `Source/WxGame/FrontEnd/WxGameFlowSubsystem.h` |
...
## 확장 포인트 / 규약
- 새 게임플레이 구성은 `AWxGameMode` 의 BP 로 만들고(폰·컨트롤러) 맵 WorldSettings 의 GameModeOverride 로 고른다. 같은 맵 다른 구성 실험은 엔진 URL 옵션 `?game=` 으로 한다.
- 새 캐릭터·컨트롤러는 대상 프레임워크 클래스를 상속한다. 플레이어 단위 도메인 컴포넌트는 `AWxPlayerController` 생성자에, 판 단위는 `AWxGameState` 생성자에 기본 서브오브젝트로 둔다. 띄울 HUD·시작 아이템처럼 컴포넌트가 소유한 구성값은 컨트롤러 BP 의 해당 컴포넌트 프로퍼티로 지정한다.
...
## 여기서부터 읽어라
1. `Source/WxGame/Framework/WxGameMode.h` — 게임이 어떻게 시작되는지(폰 선택·도착 검증)의 진입점.
