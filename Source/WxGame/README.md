...
- 구체 월드 오브젝트(체크포인트·보물상자·아이템 픽업·레이저 트랩) — [[WxWorld]]의 Gimmick 베이스를 상속해 구현
...
- 규칙: 기본 게임 모듈로 여러 플러그인을 조립하는 정상 역할 — 「WxCore 외 참조 금지」 규칙은 플러그인 전용이라 무관. ([[WxQuest]]는 코드 의존 없음 — 레벨 참조로만 연결)
...
## 확장 포인트 / 규약
- 새 캐릭터는 `AWxCharacterBase`(또는 Player/Enemy/Boss)를 상속한다. 스탯/어빌리티는 GAS 경로(AbilitySet)로 주입되며 리스폰 시 재초기화된다(PlayerState에 스탯 없음).
...
- 새 어빌리티는 [[WxCombat]]의 `UWxAbilityBase`를 상속한다. 몽타주 시점 동기화가 필요하면 짝 AnimNotify를 `Source/WxGame/AnimNotify/`에 두고 GameplayEvent로 연결한다(예: `UWxAbility_UseItem` ↔ `UWxAnimNotify_UseItem`의 Event.UseItem — 인벤토리 차감은 ServerInitiated라 서버 인스턴스만 처리).
- 새 월드 오브젝트는 [[WxWorld]]의 Gimmick 베이스를 상속하고 상호작용 컴포넌트로 노출한다. `Source/WxGame/WorldObject/`의 기존 구현(CheckPoint/TreasureChest/ItemPickup/LaserCorridor)을 본보기로.
- 글로벌 UI 데이터 소스(보스 체력바 등)는 `UWxGlobalViewModelSubsystem`의 Shell 뷰모델 Initialize/Deinitialize로 연동한다 — 뷰모델 인스턴스 자체를 갈아끼우지 않는다.
- 리플리케이션(최대 4인): 인벤토리는 `AWxPlayerController`에서 소유 클라이언트로만 복제, 장비 GE·아이템 지급·소비는 서버 권한 측에서 수행.
...
---
*문서 기준 커밋 `eb4b2a2b` · 생성일 2026-06-11 · 소스 48파일 — `/readme-writer`로 갱신*
