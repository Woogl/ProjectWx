 ...
 * 남기고 실제 선택 로직(PIE 시작 → 저장 PlayerStartTag → 최초 접속 bIsDefaultStart 체크포인트)을 이 컴포넌트가 소유한다.
...
public:
	 ...
	 * 모두 못 찾으면 nullptr 을 반환한다(호출 측 GameMode 가 엔진 기본 선택으로 폴백). 체크포인트는 있는데 최초 시작지점 미지정이면 에러 로그를 남긴다.
	 */
