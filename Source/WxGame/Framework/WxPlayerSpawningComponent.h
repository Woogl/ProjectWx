 ...
 *
 * ChoosePlayerStart 는 GameMode virtual 이라 컴포넌트로 완전히 옮길 수 없으므로, AWxGameMode 는 얇은 위임만
 * 남기고 실제 선택 로직(저장 PlayerStartTag → 최초 접속 "Default" 태그)을 이 컴포넌트가 소유한다.
 * Lyra 와 동일하게 ModularGameplay 의 UGameStateComponent 를 상속한다. AWxGameState 가 생성자에서 부착한다.
 */
UCLASS()
...
public:
	...
	/**
	 * 부활/시작 PlayerStart 를 선택한다: ① 저장된 PlayerStartTag(체크포인트 등) → ② 하드코딩 "Default" 태그(최초 접속 입구).
	 * 둘 다 못 찾으면 에러 로그 후 nullptr 을 반환한다(호출 측 GameMode 가 엔진 기본 선택으로 폴백).
	 */
