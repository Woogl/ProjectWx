...
public:
	...
	/** 충전형(Charges Fragment) 아이템의 현재 충전 횟수. 슬롯 모드는 바인딩 인스턴스, Def 모드는 첫 인스턴스 기준. 충전형이 아니면 0. */
	UPROPERTY(BlueprintReadOnly, FieldNotify, Category = "Wx|Inventory")
	int32 CurrentCharges = 0;

	/** 충전형 아이템의 최대 충전 횟수(Charges Fragment 의 MaxCharges). 정적값으로 Initialize 시 1회 세팅된다. 충전형이 아니면 0. */
	UPROPERTY(BlueprintReadOnly, FieldNotify, Category = "Wx|Inventory")
	int32 MaxCharges = 0;
